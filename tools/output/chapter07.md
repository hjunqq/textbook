        ...
        const option = {
            ...
            tooltip: {
                formatter: (params) => {
                    ...
                    return `水位: [数学公式]{point.value[1].toFixed(2)}m³/s`;
                }
            },
...
class MonitoringPointInfoPanel {
    ...
    async showPanel(deviceInfo, worldPosition, camera, renderer) {
        ...
        this.panels.set(`panel_[数学公式]{this.createHeaderHTML(deviceInfo, config)}</div>
            <div class="realtime-section">[数学公式]{this.createChartHTML()}</div>
        `;
