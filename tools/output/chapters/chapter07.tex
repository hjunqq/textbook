...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // Canvas纹理桥接实现 class ChartTextureBridge \{ constructor(width = 512, height = 512) \{ // 创建离屏Canvas this.canvas = document.createElement(`canvas'); this.canvas.width = width; this.canvas.height = height;
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} const chartConfig = \{ animation: false, responsive: false, plugins: \{ tooltip: \{ enabled: false \}, legend: \{ display: true \} \} \}; \textbackslash end\{lstlisting\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 地理坐标转换器 class CoordinateConverter \{ constructor(originLng, originLat) \{ this.origin = \{ lng: originLng, lat: originLat \}; \}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 监测点渲染器 class SensorRenderer \{ constructor(scene) \{ this.scene = scene; this.sensors = {[}{]}; \}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 射线检测器 class RaycastController \{ constructor(camera, renderer) \{ this.camera = camera; this.renderer = renderer; this.raycaster = new THREE.Raycaster(); \}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 交互控制器 class InteractionController \{ constructor(sensors, raycastController) \{ this.sensors = sensors; this.raycastController = raycastController; this.selectedSensor = null; \}
...
\textbackslash section\{技术要点回顾\}

\textbackslash begin\{tcolorbox\}{[}colback=blue!5!white,colframe=blue!75!black,title=Note 核心技术点
...
\textbackslash section\{7.1.1 水利监测数据分类体系\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 水位数据处理核心逻辑 class WaterLevelProcessor \{ processReading(reading) \{ // 1. 数据有效性检查：范围、格式、时序合理性 if (!this.validateReading(reading)) \{ throw new Error(`水位数据无效'); \}
...
\textbackslash section\{7.1.2 实时数据与历史数据处理机制\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 实时流处理核心架构 class RealTimeDataProcessor \{ async startProcessingStream(dataSourceId, config) \{ // 创建专用数据队列 const dataQueue = new DataQueue(\{ maxSize: config.queueSize \});
...
\textbackslash section\{7.1.3 数据质量检验与异常值处理\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 数据质量评估引擎 class DataQualityAssessment \{ async assessDataQuality(dataSet, metadata) \{ const results = \{\};
...
\textbackslash section\{7.1.4 多源异构数据标准化与融合\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 数据标准化核心处理逻辑 class DataStandardizer \{ async standardizeData(sourceData, sourceType) \{ const rule = this.conversionRules{[}sourceType{]};
...
\textbackslash section\{7.2.1 Chart.js在三维场景中的集成\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // Chart.js与Three.js集成的核心架构 class ChartIntegrationManager \{ createChart(chartId, config, position) \{ // 1. 创建离屏HTML容器 const container = this.createOffscreenContainer(chartId);
...
\textbackslash section\{7.2.2 ECharts高级图表集成\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // ECharts水利专业图表核心实现 class WaterChartsManager \{ createStageDischargeChart(chartId, data) \{ const chart = echarts.init(this.getContainer(chartId));
...
\textbackslash section\{7.2.3 时序数据的动态可视化\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 时序数据可视化核心实现 class TimeSeriesVisualization \{ constructor(container, config) \{ this.container = container; this.dataBuffer = new TimeSeriesBuffer(config.bufferSize); this.compressionEngine = new DataCompressionEngine(); this.chart = null; \}
...
// 高性能数据压缩引擎 class DataCompressionEngine \{ // LTTB算法：保持视觉特征的降采样 lttbDownsample(data, targetPoints) \{ if (data.length \textless= targetPoints) return data;
...
\textbackslash section\{7.2.4 响应式图表与移动端适配\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 响应式适配的核心实现 class ResponsiveChartAdapter \{ constructor() \{ this.breakpoints = \{ mobile: 768, tablet: 1024, desktop: 1200 \}; this.currentDevice = this.detectDevice(); \}
...
// 移动端优化图表工厂 class MobileOptimizedChartFactory \{ createMobileChart(container, data, options = \{\}) \{ const mobileConfig = \{ responsive: true, maintainAspectRatio: false, elements: \{ point: \{ radius: 3, hitRadius: 10 // 增大触控区域 \} \}, plugins: \{ legend: \{ display: false \}, // 移动端隐藏图例 tooltip: \{ enabled: true, mode: `nearest', intersect: false \} \}, scales: \{ x: \{ ticks: \{ maxTicksLimit: 5, // 限制标签数量 font: \{ size: 11 \} \} \}, y: \{ ticks: \{ font: \{ size: 11 \} \} \} \} \};
...
\textbackslash section\{7.3.1 监测点坐标转换与空间定位\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 监测点坐标转换管理器核心逻辑 class MonitoringPointPositionManager \{ geoToScenePosition(geoPosition) \{ // 1. 大地坐标转投影坐标 const projectedCoords = this.transformToProjection( geoPosition.longitude, geoPosition.latitude );
...
\textbackslash section\{7.3.2 设备状态颜色编码与图标系统\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 监测设备状态可视化核心实现 class DeviceStatusVisualizer \{ constructor() \{ // 标准状态颜色定义（遵循工业界通用标准） this.statusColors = \{ NORMAL: \{ primary: `\#4CAF50', glow: `\#A5D6A7' \}, // 绿色系 WARNING: \{ primary: `\#FF9800', glow: `\#FFCC02' \}, // 橙色系\\
ALERT: \{ primary: `\#F44336', glow: `\#FF8A80' \}, // 红色系 OFFLINE: \{ primary: `\#9E9E9E', glow: `\#E0E0E0' \} // 灰色系 \}; \}
...
\textbackslash section\{7.3.3 LOD技术在监测点渲染中的应用\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 监测点LOD渲染管理器核心实现 class MonitoringPointLODRenderer \{ constructor(config) \{ this.camera = config.camera; // LOD距离阈值配置（基于实际测试优化） this.lodThresholds = \{ DETAILED: 100, // 详细模型：\textless100米 MEDIUM: 500, // 中等模型：100-500米 SIMPLE: 2000, // 简单模型：500-2000米 BILLBOARD: 5000 // 广告牌：2000-5000米 \}; this.activeDevices = new Map(); // 当前活跃设备 \}
...
\textbf{点在视锥体内的判定算法} 使用Frustum类的containsPoint方法进行快速判定： \textbackslash begin\{lstlisting\} for each plane in frustum\_planes: if dot(point, plane.normal) + plane.constant \textgreater{} 0: return false // 点在平面外侧 return true // 点在视锥体内 \textbackslash end\{lstlisting\}
...
\textbf{动态阈值调整算法} 基于性能反馈的自适应算法： \textbackslash begin\{lstlisting\} adjustment\_factor = current\_fps / target\_fps if adjustment\_factor \textless{} 0.8: // 性能不足，降低LOD阈值 threshold \emph{= 0.9 elif adjustment\_factor \textgreater{} 1.2: // 性能充足，提升LOD阈值\\
threshold }= 1.1 \textbackslash end\{lstlisting\}
...
\textbackslash section\{7.3.4 监测点聚合与分层显示策略\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 监测点聚合显示管理器核心实现 class MonitoringPointClusterManager \{ constructor(config) \{ this.clusterRadius = config.clusterRadius \textbar\textbar{} 50; // 聚合半径（像素） this.clusters = new Map(); this.points = new Map(); this.zoomLevel = 12; // 当前缩放级别 \}
...
\textbackslash section\{7.4.1 射线投射算法与碰撞检测\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 高精度射线投射拾取管理器核心实现 class RaycastPickingManager \{ constructor(camera, scene, renderer) \{ this.camera = camera; this.raycaster = new THREE.Raycaster(); this.mouse = new THREE.Vector2(); // 性能优化：包围盒缓存和频率控制 this.boundingBoxCache = new Map(); this.lastPickTime = 0; this.pickingInterval = 16; // 约60fps限制 \}
...
转换公式：\textbackslash texttt\{NDC\_x = (screen\_x / width) * 2 - 1\}，\textbackslash texttt\{NDC\_y = -(screen\_y / height) * 2 + 1\}
...
\textbackslash section\{7.4.2 监测点信息面板设计与实现\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 监测点信息面板管理器核心实现 class MonitoringPointInfoPanel \{ constructor(container) \{ this.container = container; this.panels = new Map(); this.deviceConfigs = \{ WATER\_LEVEL: \{ title: `水位监测站', icon: `🌊', primaryMetric: `waterLevel' \}, FLOW\_METER: \{ title: `流量监测站', icon: `💧', primaryMetric: `flow' \}, PRESSURE\_SENSOR: \{ title: `压力监测点', icon: `⚡', primaryMetric: `pressure' \} \}; \}
...
\tightlist
\item
  \textbf{边界约束方程}：\textbackslash texttt\{left + panelWidth ≤ screenWidth - margin\}
\item
...
模板缓存机制显著提升面板创建性能： - \textbf{模板预编译}：将常用设备类型的面板结构预先编译为DOM模板 - \textbf{克隆优化}：使用\textbackslash texttt\{cloneNode(true)\}而非重新创建，性能提升约300\% - \textbf{差异更新}：只更新变化的数据部分，避免全量DOM操作
...
面板显示动画使用\textbackslash texttt\{cubic-bezier(0.25, 0.8, 0.25, 1)\}缓动函数，该参数基于： - \textbf{自然运动模拟}：模拟物理世界的加速-减速过程 - \textbf{注意力引导}：适度的弹性效果吸引用户注意但不过度干扰 - \textbf{时间感知优化}：300ms的动画时长位于用户感知的''即时响应''阈值内
...
\textbackslash section\{7.4.3 多层级信息展示策略\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 多层级信息展示管理器核心实现 class MultiLevelInfoDisplay \{ constructor(scene, camera, renderer) \{ this.scene = scene; this.camera = camera; this.renderer = renderer;
...
用户悬停时间与信息需求强度存在非线性关系： \textbackslash begin\{lstlisting\} InfoNeed(t) = 1 - e\^{}(-t/τ) \textbackslash end\{lstlisting\}
...
系统可以根据用户的历史行为调整时间阈值： \textbackslash begin\{lstlisting\}{[}language=Javascript{]} adaptiveThreshold = baseThreshold * (1 + userSpeedFactor) \textbackslash end\{lstlisting\}
...
\textbackslash section\{7.4.4 触控设备交互优化\}
...
\textbackslash begin\{lstlisting\}{[}language=Javascript{]} // 触控设备交互优化管理器核心实现 class TouchInteractionOptimizer \{ constructor(renderer, scene, camera) \{ this.renderer = renderer; this.scene = scene; this.camera = camera; this.domElement = renderer.domElement;
...
针对三维环境中小目标的触控困难，采用自适应区域扩展： \textbackslash begin\{lstlisting\}{[}language=Javascript{]} effectiveRadius = baseRadius + (depth\_distance / max\_distance) * expansion\_factor \textbackslash end\{lstlisting\}
...
双指手势识别基于向量几何： - \textbf{捏合检测}：\textbackslash texttt\{scale\_ratio = current\_distance / initial\_distance\} - \textbf{旋转检测}：\textbackslash texttt\{angle\_change = atan2(v2.y, v2.x) - atan2(v1.y, v1.x)\} - \textbf{平移检测}：\textbackslash texttt\{translation = (center\_current - center\_initial)\}
...
不同设备的像素密度差异巨大： \textbackslash begin\{lstlisting\}{[}language=Javascript{]} adaptedSize = baseSize * (devicePixelRatio / standardDPI * scaleFactor) \textbackslash end\{lstlisting\}
