...
[数学公式]
K_p = f_k(u_p,v_p)(k=1,2,3,[LaTeX命令],m;p=1,2,3,[LaTeX命令],n)
[数学公式]
(6.1.1)

式中：*[数学公式]为第*[数学公式]*号地面点（可以是单一的点,但一般是某点及其微小邻域所划定的一个地表面元）上的第*[数学公式]*类地面特性信息的取值；[数学公式]为第*p*号地面点的二维坐标，可以是采用任一地图投影的平面坐标，或者是经纬度和矩阵的行列号等；*m*（*m*大于等于1)为地面特性信息类型的数目；*n*为地面点的个数。当上述函数的定义域为二维地理空间上的面域、线段或网络时，*n*趋于正无穷大；当定义城为离散点集时，*n一般为有限正整数。

在式(6.1.1)中，当*m*=1且人为对地面高程的映射，([数学公式])为矩阵行列号时，式(6.1.1)表达的数宁地面棋型即所谓的数字高程模型（DEM)。显然，DEM是DTM的一个子集。实际上，DEM是DTM中最基本的部分，它是对地球表面地形
...
[数学公式]
V_=(X_i,Y_i,Z_i)(i=1,2,3,[LaTeX命令],n)
[数学公式]
 (6.1.2)

式中：[数学公式]是平面坐标, [数学公式]是([数学公式])对应的高程。当该序列中各平面向量的平面位置呈规则格网排列时,其平面坐标可省略,此时DEM就简化为一维向量序列[数学公式]。
...
环境光是指光线在场景中经过复杂传播之后，形成的弥漫于整个空间的光线，其光照方程可以表示为：[数学公式]，其中[数学公式]表示环境光亮度，[数学公式]表示环境光反射系数，[数学公式]表示物体表面呈现的亮度。

（2）漫反射

漫反射是指物体表面向周围辐射等强度的光照，通常这些物体的表面是粗糙并且无光泽的。满反射的光照模型可以表示为：[数学公式]；其中[数学公式]表示点光源的亮度，[数学公式]表示漫反射系数，[数学公式]表示入射角。
...
[数学公式]
d=[LaTeX命令] [LaTeX命令] [LaTeX命令]{2} (6.1.3)
[数学公式]
式中：[数学公式]为测量距离；表示光速（300000km/s）。
...
设O点在像平面坐标系中的坐标为[数学公式]，在像空间坐标系中的坐标为[数学公式]，则像平面坐标系和像空间坐标系的转换关系为：

[数学公式]
[LaTeX命令]\{ {[LaTeX命令]{*{20}{c}}
{x = x' - {x_0}}\\
{y = y' - {y_0}}
[LaTeX命令]} [LaTeX命令].
[数学公式]
...
[数学公式][LaTeX命令]}{{[LaTeX命令] t}} + [LaTeX命令]}{{[LaTeX命令] x}} + [LaTeX命令]}{{[LaTeX命令] y}} = R[数学公式] (6.1.5)
...
式中：[数学公式]表示高程中误差；*[数学公式]*表示模型高程；*[数学公式]*表示同一位置的实际高程；*[数学公式]*为高程点个数。

[数学公式] (6.3.2)

式中：[数学公式]表示样本方差；*[数学公式]*为样本数。
...
[数学公式]h = [LaTeX命令]}{{[LaTeX命令]\| {l + v} [LaTeX命令]\|}}[数学公式]6.5.2)
...
式中：[数学公式]为光线出射方向向量；[数学公式]为光线入射方向向量；*n*为*p*点所在平面的法线向量；[数学公式]为从[数学公式]方向上观察，光线投射到点*p*上反射出来的辐照度；[数学公式]为BRDF，或者双向反射分布函数，它的作用是基于表面材质属性来对入射辐射率进行缩放或者加权；[数学公式]为*p*点入射光亮度；[数学公式]为入射方向半球的积分（可以理解为无穷小的累加和)。目前常用的BRDF模型为Cook-Torrance BRDF模型，兼有漫反射与镜面反射两个部分：

[数学公式] (6.5.4)

式中：[数学公式]为入射光线中被折射部分的能量所占的比率；[数学公式]为被反射部分的比率；[数学公式]为漫反射部分；[数学公式]为BRDF镜面反射部分。[数学公式]用如下公式进行计算：

[数学公式]f_{lambert}=[LaTeX命令]{[LaTeX命令]}[数学公式] (6.5.5)

式中：*c*为表面颜色，除以[数学公式]是为了对漫反射光进行标准化。

BRDF镜面反射方程形式如下：

[数学公式]f_{cook-torrance}=[LaTeX命令]{4(w_0[LaTeX命令] n){w_i [LaTeX命令] n}}[数学公式] (6.5.6)
...
曝光控制在HDR渲染中又被称为调和映射，调和映射的方法决定了最终的图像质量。一些映射方法虽然可以将高动态的像素值映射到(0,1)上，但难以模拟人眼适应不同亮度场景的特性，因此需要选择更好的映射方法。假定某一像素点[数学公式]的亮度为[数学公式]，则其计算公式为：

[数学公式] (6.5.8)

式中：[数学公式]分别为某一像素点红、绿、蓝三个通道的颜色，各分量的权重可根据人眼对不同光的敏感程度得到。

原始图像的平均亮度为：

[数学公式] (6.5.9)

式中：[数学公式]是一个较小的常数，用以防止对数求解结果趋近于负无穷；*N*为图像中总的像素个数；[数学公式]为某一点像素的亮度值。

对于原始图像中的任一点像素点[数学公式]，存在如下的映射关系：

[数学公式]{L_{scaled}}[LaTeX命令]( p [LaTeX命令]) = [LaTeX命令]}{{Lu{m_{ave}}}}[数学公式](6.5.10)

式中：[数学公式]为常数，其大小决定了映射后的场景整体明暗程度；[数学公式]为映射后的值，为使其值处于（0,1)上，可以用如下公式进行最终的映射：
