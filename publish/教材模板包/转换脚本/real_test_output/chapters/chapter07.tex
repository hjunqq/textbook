...
=== ""
    - \important{}InfluxDBTimescaleDB
    - \important{}PostGISMongoDB
    - \important{}RedisMemcached
    - \important{}HadoopMinIO
    
=== ""
    - \important{2D}EChartsD3.jsChart.js
    - \important{3D}Three.jsCesiumBabylon.js
    - \important{}LeafletOpenLayersMapBox
    - \important{WebGL}GPULOD
    
=== ""
    - \important{}ElasticsearchApache Spark
    - \important{}
    - \important{}Vue.jsReact
    - \important{}WebSocketServer-Sent Events
\end{verbatim}
...
\footnote{Hall D L, Llinas J. An introduction to multisensor data
  fusion{[}J{]}. Proceedings of the IEEE, 1997, 85(1): 6-23.}ISO/TC
211\footnote{Castanedo F. A review of data fusion techniques{[}J{]}. The
  Scientific World Journal, 2013, 2013: 704504.}

\hypertarget{section-25}{%
\subsection{7.1.1}\label{section-25}}
...
// 
const DataCodeStandard = {
    // (2) + (2) + (2) + (4)
    stationCode: "11010001",
    
    // (1) + (3) + (2)
    elementCode: "W00101", // W- 001- 01-
    
    // YYYYMMDDHHMISS
    timeCode: "20240315143000",
    
    // 
    dataCode: \code{${stationCode}_${elementCode}_${timeCode}}
};
...
\subsection{7.1.2}\label{section-31}}
...
def assess_data_completeness(data_series, expected_interval=60):
    ...
    completeness_score = min(100, (actual_count / expected_count) \highlight{ 100)
...
def statistical_anomaly_detection(data, method='zscore', threshold=3):
    ...
    if method == 'zscore':
        ...
        upper_bound = Q3 + threshold \highlight{ IQR
        ...
        def grubbs_test(data, alpha=0.05):
            ...
            critical_value = (n-1) \highlight{ np.sqrt(t_critical\important{2 / (n}(n-2) + t_critical}2))
...
\subsection{7.1.3}\label{section-37}}
...
\subsection{7.1.4}\label{section-40}}
...
\subsection{7.1.5}\label{section-42}}
...
\subsection{7.1.2}\label{section-48}}
...
\subsection{7.1.3}\label{section-59}}
...
\subsection{7.1.4}\label{section-68}}
...
\subsection{7.1.5}\label{section-75}}
...
def assess_data_completeness(data_series, time_range):
    ...
    completeness_rate = actual_count / expected_count \highlight{ 100
...
\[MAPE = \frac{1}{n}\sum_{i=1}^{n}\left|\frac{x_i - y_i}{y_i}\right| \times 100\%\]
(7-3)
...
def statistical_anomaly_detection(data_series):
    ...
    lower_bound = mean_val - 3 \highlight{ std_val
...
\section{}\label{section-88}}

\hypertarget{section-89}{%
\subsection{}\label{section-89}}

\hypertarget{section-90}{%
\subsection{7.2.1}\label{section-90}}
    ...
    def propagate_uncertainty(self, measurements, sensor_ids):
        ...
        for i, (value, sensor_id) in enumerate(zip(measurements, sensor_ids)):
            if sensor_id in self.uncertainty_models:
                ...
                total_variance += weight**2 \highlight{ variance
                total_weight += weight
...
\subsection{7.2.2}\label{section-96}}
...
\subsection{7.2.3}\label{section-99}}
...
class SpatialInterpolator:
    ...
    def inverse_distance_weighting(self, known_points, known_values, target_points, power=2):
        ...
        for target_point in target_points:
            ...
            # 
            prediction = np.sum(weights \highlight{ known_values) / weights_sum
...
\subsection{7.2.1}\label{section-105}}
        ...
        tooltip: {
            ...
            formatter: function(params) {
                if (params.seriesName === '') {
                    return \code{: ${params.value[0]}<br/>
                           : ${params.value[1]}<br/>
                           : ${params.data.anomalyScore}};
                }
                return \code{: ${params.value[0]}<br/>: ${params.value[1]}};
            }
        }
    };
...
class MonitoringStationMap {
    ...
    initStationLayers() {
        ...
        this.clusterGroup = L.markerClusterGroup({
            iconCreateFunction: function(cluster) {
                ...
                return L.divIcon({
                    html: \code{<div class="cluster-icon">${count}</div>},
    ...
    createStationMarker(station) {
        ...
        const icon = L.divIcon({
            ...
            html: \code{<div class="marker-icon" style="background-color: ${color}">
...
class MultiScaleTimeVisualization:
    ...
    def plot_main_timeline(self, ax, parameter):
        ...
        ma_30 = self.data[parameter].rolling(window=30\highlight{24).mean()  // 30
...
function createCyclicalAnalysis(data, parameter) {
    const config = {
        ...
        xAxis: [
            {
                type: 'category',
                data: Array.from({length: 24}, (_, i) => \code{${i}:00}),
                axisPointer: {
                    type: 'shadow'
                }
            }
    ...
    config.xAxis.push(
        ...
        {
            type: 'category', 
            data: Array.from({length: 12}, (_, i) => \code{${i+1}}),
            position: 'bottom',
            offset: 120
        }
    );
...
class PositioningSystem:
    ...
    def relative_positioning(self, station):
        ...
        return {
            'offset_x': dx \highlight{ 111320 } cos(radians(station['latitude'])),
            'offset_y': dy * 111320,
            'offset_z': dz,
...
class ElevationManager {
    ...
    createDynamicElevation(station, animationDuration = 2000) {
        ...
        const animate = (currentTime) => {
            ...
            station.currentHeight = startHeight + 
                (endHeight - startHeight) \highlight{ easeProgress;
    ...
    easeInOutCubic(t) {
        return t < 0.5 ? 4 } t \highlight{ t } t : 1 - Math.pow(-2 \highlight{ t + 2, 3) / 2;
    }
}
\end{lstlisting}

\hypertarget{section-145}{%
\subsection{7.3.2}\label{section-145}}

\hypertarget{section-146}{%
\subsubsection{}\label{section-146}}

\hypertarget{section-147}{%
\paragraph{}\label{section-147}}
    ...
    createBreathingEffect(station, options = {}) {
        ...
        const animate = (timestamp) => {
            ...
            const opacity = minOpacity + 
                (maxOpacity - minOpacity) \highlight{ 
                (Math.sin(cycle } Math.PI \highlight{ 2) + 1) / 2;
...
class MonitoringPointClustering {
    ...
    buildClusters(stations, screenPositions) {
        ...
        screenPositions.forEach((item, index) => {
            ...
            const cluster = {
                id: \code{cluster_${clusters.length}},
    ...
    createClusterImage(count, size, color) {
        ...
        // 
        ctx.beginPath();
        ctx.arc(size / 2, size / 2, size / 2 - 2, 0, 2 \highlight{ Math.PI);
        ctx.fillStyle = color;
        ctx.fill();
        ...
        // 
        ctx.fillStyle = 'white';
        ctx.font = \code{bold ${size / 3}px Arial};
...
class DynamicDataLabels {
    ...
    createDataLabel(station, dataType) {
        const labelId = \code{${station.id}_${dataType}_label};
    ...
    formatDataValue(value, dataType) {
        const formatters = {
            'water_level': (val) => \code{${val.toFixed(2)}m},
            'flow_rate': (val) => \code{${val.toFixed(1)}m³/s},
            'rainfall': (val) => \code{${val.toFixed(1)}mm},
            'temperature': (val) => \code{${val.toFixed(1)}°C},
            'water_quality': (val) => \code{${val.toFixed(1)}},
            'pressure': (val) => \code{${val.toFixed(2)}MPa}
        };
...
class StatusIndicator {
    ...
    initializeStatusDefinitions() {
        return {
            ...
            alert: {
                warning: { priority: 4, color: '#fa8c16', icon: '\faExclamationTriangle' },
            ...
            data: {
                ...
                invalid: { priority: 5, color: '#f5222d', icon: '\faTimes' }
            }
        };
    }
    ...
    createRingIndicator(status, size = 20) {
        ...
        if (status.priority >= 4) {
            ctx.beginPath();
            ctx.arc(centerX, centerY, radius - 6, 0, 2 \highlight{ Math.PI);
    ...
    createPulseIndicator(status, size = 24) {
        ...
        for (let i = 0; i <= 20; i++) {
            const progress = i / 20;
            const scale = 1 + Math.sin(progress } Math.PI) \highlight{ 0.5;
            const opacity = 1 - progress } 0.5;
            
            keyframes.push({
                transform: \code{scale(${scale})},
                opacity: opacity,
                offset: progress
            });
        }
    ...
    createStatusTransition(oldStatus, newStatus, duration = 800) {
        return {
            ...
            phases: [
                {
                    // 
                    duration: duration \highlight{ 0.3,
                    opacity: { from: 1, to: 0 },
                    scale: { from: 1, to: 0.8 }
                },
                {
    ...
    isPointInPolygon(point, polygon) {
        ...
        for (let i = 0, j = vertices.length - 1; i < vertices.length; j = i++) {
            if (((vertices[i].latitude > point.latitude) !== (vertices[j].latitude > point.latitude)) &&
                (point.longitude < (vertices[j].longitude - vertices[i].longitude) \highlight{ 
...
class BufferQuery {
    ...
    createBufferZone(centerPoint, radius, options = {}) {
        const bufferId = \code{buffer_${Date.now()}};
...
\subsection{7.4.2}\label{section-169}}
...
class QueryBuilder {
    ...
    toSQL() {
        ...
        this.conditions.forEach((condition, index) => {
            if (index > 0) {
                sql += \code{ ${condition.logicalOperator} };
            }
    ...
    conditionToSQL(condition) {
        ...
        switch (operator) {
            case 'BETWEEN':
                return \code{${field} BETWEEN ${value[0]} AND ${value[1]}};
            case 'IN':
                const valueList = value.map(v => \code{'${v}'}).join(', ');
                return \code{${field} IN (${valueList})};
            case 'LIKE':
                return \code{${field} LIKE '%${value}%'};
            default:
                return \code{${field} ${operator} '${value}'};
        }
    }
...
class AdvancedFilterSystem {
    ...
    createPresetFilters() {
        return {
            ...
            recentAnomalies: () => {
                const now = Date.now();
                const yesterday = now - 24 \highlight{ 60 } 60 \highlight{ 1000;
...
\subsection{7.4.3}\label{section-172}}
...
class TimeRangeSelector {
    ...
    initializePresetRanges() {
        ...
        return {
            'last1h': {
                label: '1',
                start: new Date(now.getTime() - 60 } 60 \highlight{ 1000),
                end: now
            },
            'last6h': {
                label: '6',
                start: new Date(now.getTime() - 6 } 60 \highlight{ 60 } 1000),
                end: now
            },
            'last24h': {
                label: '24',
                start: new Date(now.getTime() - 24 \highlight{ 60 } 60 \highlight{ 1000),
                end: now
            },
            'last7d': {
                label: '7',
                start: new Date(now.getTime() - 7 } 24 \highlight{ 60 } 60 \highlight{ 1000),
                end: now
            },
            'last30d': {
                label: '30',
                start: new Date(now.getTime() - 30 } 24 \highlight{ 60 } 60 \highlight{ 1000),
                end: now
            },
            'thisMonth': {
                label: '',
            ...
            <div class="time-range-selector">
                <div class="preset-buttons">
                    ${Object.entries(this.presetRanges).map(([key, range]) => 
                        }<button class="preset-btn" data-range="${key}">${range.label}</button>\code{
                    ).join('')}
                </div>
    ...
    formatDateTimeLocal(date) {
        ...
        return \code{${year}-${month}-${day}T${hours}:${minutes}};
    }
...
class HistoricalDataRetrieval {
    ...
    aggregateData(data, resolution) {
        const groupInterval = resolution === 'day' ? 24 } 60 \highlight{ 60 } 1000 : 7 \highlight{ 24 } 60 \highlight{ 60 } 1000;
    ...
    generateCacheKey(stationId, startTime, endTime, resolution) {
        return \code{${stationId}_${startTime.getTime()}_${endTime.getTime()}_${resolution}};
    }
}
\end{lstlisting}
