\section{Vue.js基础}\label{vue.jsux57faux7840}

本部分介绍Vue.js的基础知识，包括Vue实例、模板语法、计算属性、侦听器以及条件渲染和列表渲染等核心概念，这些是开发智慧水利平台前端应用的基础。

\subsection{2.1 Vue实例}\label{vueux5b9eux4f8b}
...
\tightlist
\item
  \textbf{el}: 指定Vue实例挂载的DOM元素，可以是CSS选择器或DOM对象
\item
  \textbf{data}: 存储数据的对象，数据会被Vue转换为响应式的
\item
  \textbf{computed}:
  计算属性，基于依赖进行缓存，只有依赖变化时才重新计算
\item
  \textbf{methods}: 存储方法的对象，可以在模板中通过事件指令调用
\item
  \textbf{watch}: 侦听器，用于响应数据变化并执行异步操作或复杂逻辑
\item
  \textbf{created、mounted等}:
  生命周期钩子，在实例生命周期的不同阶段调用
\end{enumerate}

在智慧水利平台中，Vue实例通常用于管理水文站点的数据、状态和行为，例如监控水位变化、处理预警逻辑等。

\subsection{2.2 模板语法}\label{ux6a21ux677fux8bedux6cd5}

Vue.js使用基于HTML的模板语法，允许开发者声明式地将DOM绑定到Vue实例的数据。Vue会自动建立数据与DOM之间的关联，实现响应式更新。

\subsubsection{2.2.1 文本插值}\label{ux6587ux672cux63d2ux503c}

使用双大括号\passthrough{\lstinline!\{\{ ... \}\}!}在文本中插入数据：

\begin{lstlisting}[language=HTML]
<span>水位: {{ waterLevel }}米</span>
\end{lstlisting}

\subsubsection{2.2.2 指令}\label{ux6307ux4ee4}

Vue提供了许多内置指令，以\passthrough{\lstinline!v-!}为前缀：

\begin{itemize}
\item
  \textbf{v-bind}: 绑定HTML属性
...
\end{lstlisting}
\item
  \textbf{v-if/v-else/v-else-if}: 条件渲染
...
\subsubsection{2.2.3
智慧水利平台中的模板应用示例}\label{ux667aux6167ux6c34ux5229ux5e73ux53f0ux4e2dux7684ux6a21ux677fux5e94ux7528ux793aux4f8b}
...
\subsection{2.3
计算属性与监听器}\label{ux8ba1ux7b97ux5c5eux6027ux4e0eux76d1ux542cux5668}

计算属性和监听器是Vue.js中处理复杂逻辑的两种重要机制。

\subsubsection{2.3.1 计算属性}\label{ux8ba1ux7b97ux5c5eux6027}
...
\subsubsection{2.3.2 监听器}\label{ux76d1ux542cux5668}
...
\subsection{2.4
条件渲染与列表渲染}\label{ux6761ux4ef6ux6e32ux67d3ux4e0eux5217ux8868ux6e32ux67d3}

条件渲染和列表渲染是Vue.js中常用的两种DOM操作方式，适用于智慧水利平台中的多种场景。

\subsubsection{2.4.1 条件渲染}\label{ux6761ux4ef6ux6e32ux67d3}

Vue提供了\passthrough{\lstinline!v-if!}、\passthrough{\lstinline!v-else-if!}、\passthrough{\lstinline!v-else!}和\passthrough{\lstinline!v-show!}指令进行条件渲染。

\begin{itemize}
\tightlist
\item
  \passthrough{\lstinline!v-if!}/\passthrough{\lstinline!v-else-if!}/\passthrough{\lstinline!v-else!}:
  根据条件真假，完全渲染或移除元素
\item
  \passthrough{\lstinline!v-show!}: 根据条件真假，切换元素的CSS
  \passthrough{\lstinline!display!}属性
\end{itemize}
...
\textbf{适用场景}： - \passthrough{\lstinline!v-if!}:
适用于条件很少改变的场景（如权限控制） -
\passthrough{\lstinline!v-show!}:
适用于需要频繁切换的场景（如面板展开/折叠）

\subsubsection{2.4.2 列表渲染}\label{ux5217ux8868ux6e32ux67d3}

使用\passthrough{\lstinline!v-for!}指令基于数组渲染列表，通常与\passthrough{\lstinline!:key!}属性一起使用，以帮助Vue高效更新DOM。
...
\tightlist
\item
  \textbf{显示不同级别的预警信息}：根据水位、流量等数据的不同状态，显示不同级别的预警信息
\item
  \textbf{展示监测站点列表}：将后端获取的站点数据以列表或网格形式展示
\item
  \textbf{分区域显示站点信息}：按照行政区划或流域划分展示不同区域的水文站点
\item
  \textbf{根据用户权限显示不同功能}：基于用户角色显示或隐藏特定的操作按钮和管理选项
\item
  \textbf{数据仪表盘组件的动态加载}：根据用户配置，动态渲染不同的数据展示组件
\end{enumerate}
