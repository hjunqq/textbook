...
【第四步：新增图表与代码清单的额外要求】
门禁在 O6 之后新增了三条检查，本包新增的每个浮动体都必须满足：
  · 就近引出——每个新增图/表/公式的 \label 必须在它自己前后 40 行以内被 \ref 到。
    章末集中罗列不算数，门禁会逐个点名。
  · 引导语不得套模板——同一句话（抹掉 label 后归一化）复用 3 次以上直接判失败。
    引导语要说明这一张图表回答什么问题，因此不同图表的引导语必然不同。
    自检办法：把这句话搬到另一张图上，如果照样通顺，就说明它什么也没说。
  · 一段里最多 3 个 \ref，超过判为堆砌。

【第五步：本包不许出现的三种"达标但没干活"】
前面有一个工作包连续三次用低成本方式满足了指标，代价是整包返工。以下三种情形，
门禁未必拦得住，但人工抽查一定会退回：
  · 缩写式回收——把手稿里 19,456 字的一节压成 3,000 字塞进来，字数达标而内容没了。
    判断标准：源文件里讲了十个知识点，你的版本是不是只剩一句罗列。
  · 凑数式载体——为满足"每 1200 字一个载体"而塞进三行小表或截断的代码片段。
    新增的表必须有实际对照价值，新增的代码必须能独立运行。
  · 空转式行文——"具有重要意义""为…提供了有力支撑""随着…的不断发展"。
    这类句子一经发现，整段退回重写。

如果本包的目标内容在 L1/L2 里确实不存在、需要新写，直接说明，不要假装是回收。
```
