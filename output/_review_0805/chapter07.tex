...
本书以Apache ECharts为图表主线。它是Apache软件基金会项目提供的开源JavaScript可视化库，而不是“百度开源项目”的现行表述\cite{apacheECharts}。Chart.js可用于轻量页面，Three.js负责三维场景；二者通过对象标识和时间范围与图表协同。
...
屏幕点击首先转换为归一化设备坐标，再由相机生成射线，与场景对象求交。Three.js的\texttt{Raycaster}已经处理常见网格与点对象；拾取器应在构造时保存场景引用，不能调用未定义的全局变量。
