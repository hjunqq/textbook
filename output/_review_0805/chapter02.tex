...
敏捷强调通过短周期、可运行软件和持续反馈适应变化\cite{agile2001}。核心参与者是业务用户、客户代表与跨职能交付团队，不应表述为“运维人员高度参与”即可代表敏捷。水利项目采用敏捷时，仍需保留法规、标准、架构、安全和验收所要求的文档与审批。
...
系统流程图关注业务步骤、判断、输入输出和责任衔接。图\ref{fig:order-process-flowchart}描述用水计划从申报到执行反馈的闭环：资料不完整时从“完整性检查”退回“提交计划”；审批不通过时返回“修订计划”，而不是逆转告警处置顺序。
...
数据字典定义数据流、数据存储和数据元素的结构与约束。它不应写成“计划编号（计划编号）”之类的同义反复，而应给出编码、类型、单位、来源和校验规则。
...
数据流也要定义。例如“执行回执”至少包含任务ID、设备ID、指令时间、实际启闭时间、目标流量、实测流量、质量码和回执状态。描述水工设备动作时统一使用“闸门启闭”。
