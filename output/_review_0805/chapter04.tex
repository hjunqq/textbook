...
Grid适合仪表盘的二维区域布局，Flexbox适合工具栏等一维排列。响应式设计不能只缩小字号，还应调整列数、触控目标和信息优先级。上例在窄屏上改为单列，完整且不存在未闭合的媒体查询或选择器。
...
本章使用Vue 3.4+组合式API。\texttt{ref}适合基本值，\texttt{reactive}适合对象，\texttt{computed}表达派生状态；组件通过props接收数据，通过emit通知父组件。以下单文件组件没有省略语法结构。
