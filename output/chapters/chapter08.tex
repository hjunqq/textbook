...
\section{8.1 水利工程安全监测平台概述}\label{ux6c34ux5229ux5de5ux7a0bux5b89ux5168ux76d1ux6d4bux5e73ux53f0ux6982ux8ff0}
...
其中： - T\_i：第i个数据源的采集时间 - W\_i：第i个数据源的等待时间 - R\_i：第i个数据源占用的资源 - R\_max：系统最大可用资源
...
其中： - X̂(k\textbar k)：k时刻的最优估计 - K(k)：卡尔曼增益 - Z(k)：k时刻的观测值 - H：观测矩阵
...
minimize: F(x) = {[}f₁(x), f₂(x){]} where: f₁(x) = processing\_time, f₂(x) = -accuracy\_score
...
\textbf{应急响应决策系统的决策理论与管理科学基础深度解析}
...
时间窗口模型：\passthrough{\lstinline!T\_total = T\_detection + T\_decision + T\_action!}
...
\textbf{第一类错误（误报）}：P(reject H0 \textbar{} H0 true) \textbf{第二类错误（漏报）}：P(accept H0 \textbar{} H1 true)
...
对于不同类型指标： - \textbf{CPU使用率}：变化频率0.1Hz，采样频率≥0.2Hz - \textbf{内存使用率}：变化频率0.01Hz，采样频率≥0.02Hz - \textbf{网络流量}：变化频率1Hz，采样频率≥2Hz
