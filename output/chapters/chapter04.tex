...
图\ref{fig:ch04-document-flow}展示同一文档的两条处理路径：DOM 与 CSSOM 支持视觉渲染，DOM 等信息还用于生成可访问性树，供屏幕阅读器等辅助技术读取角色、名称和状态。页面地标来自文档语义；调试时先判断问题属于结构、样式还是脚本，再选择对应工具。
...
图\ref{fig:ch04-responsive-breakpoints}给出窄视口和三个宽视口布局示例，宽度以 CSS 像素计。示例在 1920 像素时安排侧栏、主图和告警栏，在 2560 与 3840 像素时增加留白和可见信息；窄视口优先保留水位、预警和更新时间，折叠次要筛选条件。实际断点仍需按内容宽度验证，断点之间采用流式尺寸。
...
浏览器事件沿 DOM 树传播，通常分为捕获、目标和冒泡三个阶段。捕获从\texttt{window}向目标节点下行，目标阶段执行目标监听器，冒泡再从目标向祖先上行。注册监听器时可在第三个参数中设置\texttt{capture: true}；祖先节点上的监听器默认接收冒泡事件，目标节点上的监听器则在目标阶段执行。事件对象的\texttt{target}是最初触发的节点，\texttt{currentTarget}是当前正在执行监听器的节点，两者不同是理解事件委托的关键。
...
受保护的测站列表需要前端携带令牌，并由后端依次完成验签和权限检查。第5章的登录控制器返回访问令牌，令牌至少包含主体、发行者、有效期、令牌类型和权限代码；前端登录页保存访问令牌，\texttt{request.js}在每次受保护请求中注入 Bearer 头，后端的 \texttt{SecurityFilterChain} 和 JWT 过滤器验签后把主体放入安全上下文，授权规则再决定是否允许执行控制器或业务方法。图\ref{fig:ch04-jwt-integration}把这条链路与失败分支并列画出，调试时可逐段定位是凭据错误、令牌过期、权限不足还是网络转发问题。
...
组件在创建、挂载、更新和卸载阶段提供生命周期钩子。Vue 3中卸载阶段使用\texttt{beforeUnmount}/\texttt{unmounted}或组合式API的\texttt{onBeforeUnmount}/\texttt{onUnmounted}，不使用Vue 2的\texttt{beforeDestroy}/\texttt{destroyed}。图\ref{fig:vue-lifecycle}以组合式 API 展示挂载、运行与卸载过程；响应式状态引起 DOM 更新时进入更新分支，该分支可以执行零次或多次。需要在组件内启动的定时刷新、消息订阅和三维场景实例，都应在挂载阶段建立、在卸载阶段释放；忘记释放是监测页面反复切换之后内存持续增长的最常见原因。
...
\begin{tikzpicture}[>=Stealth,thick,node distance=0.85cm and 1.0cm]
  ...
  \node[below=.45cm of use,font=\scriptsize,text=TextGray,align=center]
    {保存资源句柄；卸载时清除定时器、取消订阅与未完成请求};
\end{tikzpicture}
