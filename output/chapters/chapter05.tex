...
智慧水利后端负责接入监测数据、执行业务规则、维护一致性并向前端提供接口。本章统一采用Java 17、Spring Boot 3.2+、Spring Security 6、Jakarta Persistence 3.1与Jakarta Servlet 6.x；JWT示例按jjwt 0.11.x接口编写。框架配置、注解语义与版本迁移细节可查阅对应版本的官方参考文档\cite{springBootDocs,springSecurityDocs,jakartaPersistence}；运行示例时应先核对 Java、依赖与容器版本。分布式事务和领域驱动设计作为选学内容，不作为首次后端课程的前置要求。本章延续第3章的架构决策：模块化单体内部按控制器、服务、数据访问分层，观测写入经Kafka削峰后进入TimescaleDB；案例水库峰值每秒10条观测、50并发查询的负载数字，是本章连接池、缓存与分页设计的直接依据。
...
分页、排序和过滤也要形成稳定的 URL 契约。\texttt{page}表示页码，起始值需由接口契约约定；下面示例采用从0开始的\texttt{page}、正数\texttt{size}，排序使用\texttt{sort=occurredAt,desc}，过滤使用\texttt{assetId}、\texttt{quality}和时间范围。服务端限制\texttt{size}上限，拒绝未知排序字段，返回\texttt{content}、\texttt{page}、\texttt{size}、\texttt{totalElements}和\texttt{totalPages}，这样前端可以显示总页数而不是用当前页长度猜测。
...
\paragraph{业务问题}第4章的页面一直对着教学接口开发。现在要让\texttt{GET /api/assets}由自己写的 Java 程序应答，而且沿用页面已经使用的字段和响应格式。首先返回固定对象列表，观察 Java 方法与 HTTP 路径的映射；再处理路径参数与查询参数，把错误体做成契约规定的样子；最后用第4章的页面和教学接口的故障注入表反过来验证它。数据库到 5.4 节才接入。
...
目录结构还应服务于模块边界。\texttt{common}保存跨模块共享的值对象和错误码，模块专用工具保留在各自目录；\texttt{api}只放外部 DTO 和客户端契约，\texttt{domain}不依赖 Spring Web；数据库迁移文件按版本命名并与发布记录关联。新增预警服务时，先复制边界和测试结构，再决定是否拆分进程，避免目录先于业务边界膨胀。
...
实体映射需要同时定义对象身份、数值精度、生命周期和并发更新规则。水位属于有单位的测量值，使用\texttt{BigDecimal}并设置数据库精度，避免在累计统计时把二进制浮点误差当作工程变化；\texttt{occurredAt}记录设备采样时间，\texttt{createdAt}和\texttt{updatedAt}记录服务器侧生命周期。\texttt{version}由 JPA 在更新时递增，控制器将它映射为响应中的版本，服务层在替换前比较客户端版本。清单\ref{lst:ch05-jpa-entity}通过实体访问器提供读数，DTO 的\texttt{ReadingResponse.from}能够通过公开的只读方法取值，而不会依赖反射或直接暴露字段。
...
测站与读数是一对多关系，读数与测站是多对一关系。多对一侧通常使用懒加载，避免每读取一条读数都立即查询测站；一对多集合也使用懒加载，并通过专门的查询决定何时批量取出。清单\ref{lst:ch05-jpa-association}展示了双向关系的维护方式：只有聚合根方法可以建立关联，集合不直接暴露给控制器，序列化层使用 DTO 打断循环引用。
...
懒加载将关联数据的读取推迟到访问该关联时；查询用例需要安排这一时机。若在事务外调用集合访问器，会出现\texttt{LazyInitializationException}；若为了避免异常把所有关系改成\texttt{EAGER}，列表接口又会产生大量无关数据和更大的连接结果集。更安全的做法是在服务层打开事务，在查询中声明需要的关联，随后立即映射成 DTO 并结束实体生命周期。日志、JSON 序列化和模板渲染都不应隐式触发懒加载。
...
Spring Data 的派生查询名称按“动词 + 属性 + 条件 + 排序”组合，例如\texttt{findByStationId\allowbreak AndQuality\allowbreak OrderByMeasuredAt\allowbreak DescIdDesc}。属性名必须与实体字段对应，重命名时编译器会提示错误；查询含义复杂或涉及时间半开区间时，优先使用\texttt{@Query}明确写出条件。所有列表接口都应限制\texttt{size}，默认排序追加唯一 ID 作为 tie-breaker，保证追加新读数时不会在页边界重复或漏掉记录。
...
最后，查询契约应写入接口文档和回归测试：字段名称、单位、时区、排序、分页边界及错误码都要有固定示例。优化后用相同输入检查返回值和错误响应，并比较执行计划与耗时，确认性能变化没有破坏接口约定。
...
脏读是读取到尚未提交的数据，读到回滚值会使预警计算产生错误；不可重复读是同一事务两次读取同一行得到不同版本；幻读是两次范围查询返回的行集合变化；丢失更新是两个写者基于旧值覆盖彼此结果。\texttt{@Version}可以发现实体级丢失更新，唯一约束和检查约束负责拒绝非法状态，隔离级别则决定范围读取和并发写入之间的可见性。提升隔离级别后仍需处理并发冲突，并配置重试上限、退避与告警。
...
清单\ref{lst:ch05-dto-mapper}把外部字段转换集中在映射边界，单位、质量码和版本号不会因为数据库字段改名而泄露到 API。\texttt{version}为后续的乐观锁校验预留位置；更新请求携带旧版本时，服务层可以在保存前检查版本是否仍然匹配，冲突返回409并提示客户端重新读取，而不是覆盖另一个值班员刚提交的数据。清单\ref{lst:ch05-command-service}把写入、订正与失效三种命令放在同一个应用服务里，三种命令共用领域校验，并分别执行相应的状态迁移。
...
清单\ref{lst:ch05-problem-advice}只把字段名和稳定错误码放入响应，具体原因留在受控日志。四个处理器产出同一形状的\texttt{\{code, message, field?\}}，与表\ref{tab:api-contract}的通用约定、教学接口以及 5.2 节控制器的格式一致，前端可使用相同的字段读取和错误提示逻辑。\texttt{field}指向第一个出错的字段，帮助前端定位输入框；对跨字段错误，服务层抛出带业务码的异常。异常处理器本身也应有测试，确保新异常不会意外回落到500，并验证响应的\texttt{Content-Type}为\texttt{application/problem+json}。
...
接口集成测试使用\texttt{@SpringBootTest}加载真实配置和安全过滤链，再用\texttt{MockMvc}发出 HTTP 请求。测试配置应使用专门的 Profile，令牌签名密钥、数据库 URL 和 Kafka 地址全部由测试环境注入，禁止读取开发机的生产变量。清单\ref{lst:ch05-mockmvc-test}把四种基本场景写成独立测试方法，并验证响应体中的错误码；无权限场景必须经过实际的过滤链，不能只调用控制器方法绕过授权。
...
检查实现时，应分别验证格式、业务规则与持久化约束；服务层需检查同类内部调用是否绕过事务代理，导致 \texttt{@Transactional} 失效；持久化层需检查 N+1 查询和事务外的懒加载；事件如果在事务提交之前发布，回滚后消息已经发了出去；消费者若没有去重的唯一约束，一次重试就会变成一次重复入库。密钥硬编码、Actuator 端点全量暴露、日志回显异常堆栈，也都应在上线前排除。
