...
智慧水利后端负责接入监测数据、执行业务规则、维护一致性并向前端提供接口。本章统一采用Java 17、Spring Boot 3.2+、Spring Security 6、Jakarta Persistence 3.1与Jakarta Servlet 6.x；JWT示例按jjwt 0.11.x接口编写。框架配置、注解语义与版本迁移细节，一律以官方参考文档为准\cite{springBootDocs,springSecurityDocs,jakartaPersistence}；书中给出的版本边界只描述编写时点的状态。分布式事务和领域驱动设计作为选学内容，不作为首次后端课程的前置要求。本章延续第3章的架构决策：模块化单体内部按控制器、服务、数据访问分层，观测写入经Kafka削峰后进入TimescaleDB；案例水库峰值每秒10条观测、50并发查询的负载数字，是本章连接池、缓存与分页设计的直接依据。

\begin{tipbox}
\paragraph{工程版本线：v1（前端只读切片） $\rightarrow$ v2（认证与观测API）}起点是第4章交付的 v1 前端。本章结束时你应交付 \textbf{v2}：为 v1 提供真实数据的后端——登录签发 JWT、受保护的测点与观测查询、事务与幂等写入——对应仓库 \texttt{backend/} 的 \texttt{JwtService}、\texttt{JwtAuthenticationFilter}、\texttt{SecurityConfig}、\texttt{AuthController}、\texttt{AssetController}、\texttt{ReadingService}；验证命令 \texttt{mvn test}，联调后前端 401 分流与登录回跳应全部生效。
\end{tipbox}
...
\paragraph{进入本章所需知识}会写 Java 类、方法、\texttt{List}与\texttt{try/catch}；读过附录B的 P2 单元（工程目录、Maven、注解与依赖注入）和 P3 单元（表、键、SQL 与事务）。两个单元末尾的自测题做得出来，本章清单里的注解与 SQL 就不需要另外查资料。
...
\begin{lstlisting}[label={lst:ch05-rest-layer},language=Java,caption={分层 REST 查询：DTO、服务与仓储接口}]
...
\begin{lstlisting}[label={lst:ch05-rest-write},language=Java,caption={监测值写入：POST、校验与201 Location}]
...
\paragraph{本节层次}核心：5.2.1、5.2.3；指导实践：5.2.2、5.2.4、5.2.6；拓展：5.2.5。核心路线只要求读完核心小节。
...
\begin{lstlisting}[label={lst:ch05-boot-dependencies},language=XML,caption={Spring Boot 3.2 起步依赖与版本管理}]
<parent>
...
配置文件按环境拆分，公共配置放在\texttt{application.yml}，开发、测试和生产差异分别放在\texttt{application-development.yml}、\texttt{application-test.yml}和\texttt{application-production.yml}。启动时使用\texttt{SPRING\_PROFILES\_ACTIVE}选择 Profile，敏感值通过环境变量或密钥管理器注入。Profile 只表达环境差异，不应把业务阈值复制成多套；案例水库的工程参数仍以第8章 8.1 参数表为唯一来源。

\begin{lstlisting}[label={lst:ch05-profile-config},language=YAML,caption={多环境配置与外部化密钥}]
...
\begin{lstlisting}[label={lst:ch05-di-lifecycle},language=Java,caption={构造器注入与 Bean 生命周期}]
@Configuration
...
工程目录应与依赖方向一致：\texttt{controller}只依赖\texttt{service}，\texttt{service}依赖领域对象和仓储接口，\texttt{repository}适配 PostgreSQL/PostGIS/TimescaleDB，\texttt{config}集中放安全、序列化和外部客户端。测试代码按同样的包结构组织，配置文件分为测试 Profile 和生产 Profile，避免在测试中意外连接真实案例水库数据库。
...
\begin{lstlisting}[label={lst:ch05-config-validation},language=Java,caption={配置属性注册与启动校验}]
...
\begin{lstlisting}[label={lst:ch05-http},language=bash,caption={按职责分层的工程目录}]
...
\begin{lstlisting}[label={lst:ch05-application},language=YAML,caption={Spring Boot应用入口与JWT配置}]
spring:
  datasource:
    url: ${DB_URL:jdbc:postgresql://localhost:5432/water}
    username: ${DB_USER:water_app}
...
\begin{lstlisting}[label={lst:ch05-query-controller},language=Java,caption={查询接口控制器}]
@RestController
...
\paragraph{本节层次}核心：5.3.1、5.3.9、5.3.10；指导实践：5.3.2、5.3.4、5.3.5、5.3.6、5.3.13；拓展：5.3.3、5.3.7、5.3.8、5.3.11、5.3.12、5.3.14。核心路线只要求读完核心小节。

\subsection{实体映射与Repository基础}

先建立实体映射与Repository，再讨论事务边界。实体标识应稳定；接口层DTO与持久化实体分离，避免数据库结构直接成为外部契约。

\begin{lstlisting}[label={lst:ch05-jpa-entity},language=Java,caption={Jakarta实体映射}]
...
\begin{lstlisting}[label={lst:ch05-jpa-association},language=Java,caption={测站与读数的懒加载关联}]
import jakarta.persistence.*;
import java.time.Instant;
import java.util.*;
...
N+1 的典型形状是先查询 N 条读数，再对每条读数访问一次测站字段，数据库日志中会出现 1 条集合查询加 N 条单项查询。数据量小的时候不明显，到了案例水库的历史查询或报表导出就会耗尽连接池。修复前先用 SQL 日志、慢查询日志或测试断言确认问题，再选择 fetch join、\texttt{@EntityGraph}、批量大小或 DTO 投影；不能仅凭“加了索引”推断 N+1 已消失。

\begin{lstlisting}[label={lst:ch05-fetch-join},language=Java,caption={用 fetch join 消除列表接口的 N+1}]
...
\begin{lstlisting}[label={lst:ch05-pageable-repository},language=Java,caption={Pageable 分页查询与服务层上限}]
...
\begin{lstlisting}[label={lst:ch05-version-audit},language=Java,caption={乐观锁与审计字段映射}]
import jakarta.persistence.*;
...
索引要由过滤、排序、连接和唯一性共同决定。测站读数列表通常先按\texttt{station\_id}过滤，再按\texttt{measured\_at}和\texttt{id}倒序，因此联合索引应保持相同的前缀顺序；质量码只有在选择性较高或统计查询频繁时才适合放入索引。索引不能替代时间分区、TimescaleDB 超表或 PostGIS 空间索引，创建后要用\texttt{EXPLAIN (ANALYZE, BUFFERS)}核对实际执行计划；执行计划算子与代价模型的解释见 PostgreSQL 官方文档\cite{postgresDocs}，超表与连续聚合的行为约束见 TimescaleDB 官方文档\cite{timescaleDocs}。

\begin{lstlisting}[label={lst:ch05-index-migration},language=SQL,caption={PostgreSQL 索引与扩展迁移}]
...
\begin{lstlisting}[label={lst:ch05-repository},language=Java,caption={事务代理、审计传播与提交后事件}]
...
\begin{lstlisting}[label={lst:ch05-propagation-service},language=Java,caption={事务传播行为的服务边界示例}]
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.*;
...
\begin{lstlisting}[label={lst:ch05-isolation-test},language=Java,caption={隔离级别与乐观锁的集成测试思路}]
...
\begin{lstlisting}[label={lst:ch05-read-only},language=Java,caption={只读查询与提交后事件的完整调用}]
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.*;
...
\begin{lstlisting}[label={lst:ch05-tx-service},language=Java,caption={增量统计的数值稳定实现}]
...
\paragraph{本节层次}核心：5.4.1；指导实践：5.4.2、5.4.3、5.4.5；拓展：5.4.4、5.4.6。核心路线只要求读完核心小节。

服务层按用例组织方法，例如“保存监测值”“确认告警”“查询测站详情”。输入DTO先通过Bean Validation完成格式校验，领域规则仍由服务或领域对象校验。统一响应不应把所有结果都包装成HTTP 200；HTTP状态码仍须表达协议层结果。

\begin{lstlisting}[label={lst:ch05-exception-handler},language=Java,caption={参数校验异常处理}]
...
\begin{lstlisting}[label={lst:ch05-crud-controller},language=Java,caption={监测读数 GET/POST/PUT/DELETE 控制器}]
@RestController
@RequestMapping("/api/readings")
...
\begin{lstlisting}[label={lst:ch05-dto-mapper},language=Java,caption={DTO 与实体的双向映射}]
...
\begin{lstlisting}[label={lst:ch05-command-service},language=Java,caption={应用服务中的写入、更新与删除规则}]
@Service
...
\begin{lstlisting}[label={lst:ch05-problem-advice},language=Java,caption={RestControllerAdvice 统一错误响应}]
@RestControllerAdvice
...
\paragraph{本节层次}指导实践：5.5.1、5.5.3、5.5.5；拓展：5.5.2、5.5.4、5.5.6、5.5.7、5.5.8。本节没有核心小节，课堂核心路线可整体跳过。
...
JWT 的声明字段、签名与校验语义以 RFC 7519 为规范依据\cite{rfc7519}。刷新采用轮换策略：每次成功使用旧 refresh jti 后立即撤销，并生成新的 refresh jti；若同一旧令牌再次出现，系统可以判定重放并撤销整个会话族。解析过期刷新令牌时只允许捕获签名已验证的\texttt{ExpiredJwtException}，还要检查过期时间、issuer、type、jti 和服务端会话，不能使用\texttt{parseClaimsJwt}或手工拼接 Claims。
...
\paragraph{本节层次}核心：5.6.1；指导实践：5.6.2、5.6.3、5.6.4、5.6.5；拓展：5.6.6。核心路线只要求读完核心小节。
...
\begin{lstlisting}[label={lst:ch05-metrics},language=Java,caption={Kafka 消息监听与告警评估}]
...
\begin{lstlisting}[label={lst:ch05-kafka-idempotency},language=Java,caption={事件去重表与消费者事务}]
import jakarta.persistence.*;
...
\paragraph{本节层次}拓展：5.7.1、5.7.2、5.7.3、5.7.4、5.7.5。本节没有核心小节，课堂核心路线可整体跳过。
...
\begin{lstlisting}[label={lst:ch05-unit-service-test},language=Java,caption={服务层单元测试：质量码与幂等决策}]
import static org.assertj.core.api.Assertions.assertThatThrownBy;
...
\begin{lstlisting}[label={lst:ch05-mockmvc-test},language=Java,caption={MockMvc 接口测试：四种基础场景},breaklines=true]
...
\begin{lstlisting}[label={lst:ch05-testcontainers},language=Java,caption={Testcontainers：隔离 PostgreSQL 集成环境}]
