...
\paragraph{进入本节所需知识}会写 Java 类、方法、\texttt{List}与\texttt{try/catch}；读过附录B的 P2 单元（工程目录、Maven、注解与依赖注入）和 P3 单元（表、键、SQL 与事务）。先完成两个单元的自测，再带着一次请求的路径、参数和响应学习后端职责；具体注解与 SQL 的使用在相应小节展开。
...
资源路径使用名词，例如\texttt{/api/assets/DAM-A-PZ-07/readings}；筛选条件放入查询参数，例如\texttt{?from=...\&to=...}。设备协议接入属于边缘网关职责，本章不展开Modbus或MQTT细节。

资源建模先回答“系统管理的对象是什么”。测站是稳定资源，监测读数是按时间追加的子资源，告警处置单是具备状态迁移的业务资源。查询最新读数可以使用\texttt{GET /api/assets/DAM-A-PZ-07/readings/latest}，查询时间范围使用\texttt{GET /api/assets/DAM-A-PZ-07/readings?from=...\&to=...}；创建一条读数使用\texttt{POST /api/readings}，请求体包含测站 ID、采样时间、数值、单位和质量码。路径表达资源层级，查询参数表达视图条件，业务动作只有在无法用资源状态表达时才使用动词，例如表\ref{tab:api-contract}中的\texttt{POST /api/warnings/w-0001/ack}。
...
写入接口要把协议校验、业务校验和持久化顺序写清。协议层检查必填字段、数值格式和时间字符串，应用服务检查测站是否存在、采样时间是否落在允许窗口、质量码是否允许参与计算，Repository 在事务中保存记录。创建成功返回\texttt{201 Created}并在\texttt{Location}头给出新资源地址；重复的幂等键返回第一次创建的响应或\texttt{409 Conflict}，不能静默创建第二条记录。清单\ref{lst:ch05-rest-write}按这个顺序写：先\texttt{@Valid}校验请求体，再由服务层判断测站与时间窗，最后返回带\texttt{Location}的 201。
...
分页、排序和过滤也要形成稳定的 URL 契约。\texttt{page}从0开始还是从1开始必须在全书统一；本章示例采用从0开始的\texttt{page}、正数\texttt{size}，排序使用\texttt{sort=occurredAt,desc}，过滤使用\texttt{assetId}、\texttt{quality}和时间范围。服务端限制\texttt{size}上限，拒绝未知排序字段，返回\texttt{content}、\texttt{page}、\texttt{size}、\texttt{totalElements}和\texttt{totalPages}，这样前端可以显示总页数而不是用当前页长度猜测。
...
API 版本化需要在兼容性和演进速度之间作出可审计的选择。对外公开的水利平台接口可以采用路径版本\texttt{/api/v1/assets}，在字段增加时保持旧字段语义不变；删除字段、改变单位或改变时间时区都属于破坏性变更，应发布\texttt{v2}并给出迁移窗口。版本号不应随着每次修复递增，补丁修复和兼容字段增加可以通过文档和变更日志说明。响应体可带\texttt{Deprecation}和\texttt{Sunset}提示，让调用方在停止旧版本前完成升级。
...
\paragraph{进入本节所需知识}先读5.1节；按附录B P2准备 Java 17与 Maven，先运行固定数据接口。


\paragraph{业务问题}第4章的页面一直对着教学接口开发。现在要让\texttt{GET /api/assets}由自己写的 Java 程序应答，而且页面一行不改。本节先返回写死的对象列表，把“一个方法怎样变成一个 HTTP 接口”看清楚；再处理路径参数与查询参数，把错误体做成契约规定的样子；最后用第4章的页面和教学接口的故障注入表反过来验证它。数据库到 5.4 节才接入。

\paragraph{进入本节所需知识}附录B的 P2 单元（工程目录、Maven、注解、依赖注入）；表\ref{tab:api-contract}前三行；能用浏览器地址栏或\texttt{curl}发一个 GET 请求。
...
契约的第三、四行需要两样东西：路径里的对象编码（\texttt{/api/assets/}\allowbreak\texttt{\{assetId\}/}\allowbreak\texttt{readings/latest}）和查询串里的时间范围（\texttt{?from=…\&to=…}）。清单\ref{lst:ch05-first-params}在同一个控制器里加两个方法。\texttt{@PathVariable}把路径片段绑定到参数，\texttt{@RequestParam}把查询参数绑定到参数并自动转换类型；找不到对象时不能返回一个空对象让页面猜，而要按契约返回 404 与\texttt{\{code, message\}}。这里用一个最小的异常类和\texttt{@ExceptionHandler}把“业务上的找不到”翻译成 HTTP 的 404；5.5 节把它扩展成全局统一的错误响应。
...
\paragraph{一个会遇到的失败}\texttt{from=2026-07-01T00:00:00+08:00}直接放进地址栏，加号会被浏览器当作空格，服务端解析时报 400，错误体却不是契约格式而是 Spring 的默认页面。原因是类型转换失败发生在进入方法之前，\texttt{@ExceptionHandler(BadRequest.class)}接不到它。两件事要做：客户端把\texttt{+}编码成\texttt{\%2B}（第4章的\texttt{URLSearchParams}会自动做）；服务端在 5.5 节用\texttt{@ControllerAdvice}接住\texttt{MethodArgumentTypeMismatchException}，统一翻译成契约错误体。这个失败值得亲手复现一次：它说明“错误体格式统一”不是一个类里加几个方法就能完成的。

\subsection{用第4章的页面和故障表验证接口}

接口验证分起点与终点两次进行。起点按表\ref{tab:ch05-first-verify}直接请求本节已实现的接口，检查固定数据、204、400与404；此时无登录端点，历史查询也只是空列表骨架。终点完成5.4节存储、5.5节全局异常处理和5.6节认证后，再把 S2 页面接入完整后端，重做表\ref{tab:ch04-stage-acceptance}的界面验收。教学接口的\texttt{teach=}由表\ref{tab:api-teach-faults}约定；真实后端用不带令牌、非法时间范围、不存在的对象和无观测对象触发相应行为。输入\texttt{from}晚于\texttt{to}应得到400且\texttt{field}为\texttt{from}；错误体尚未统一的类型转换失败在5.5节补齐。字段一致允许复用页面，认证和数据能力齐备才允许替换整个服务。
...
\paragraph{S3 阶段包与契约核对脚本。}清单\ref{lst:ch05-first-controller}与清单\ref{lst:ch05-first-params}合起来就是配套工程里的\texttt{backend/src/main/java/edu/example/lesson52/}，它是 S3 的起点：一个类、没有数据库、没有认证。它刻意放在\texttt{edu.example.lesson52}包下而不是完整工程的\texttt{edu.example.qingyuan}包下——后者会被完整工程一起扫描，两个\texttt{/api/assets}映射冲突，启动即失败。表\ref{tab:ch05-first-verify}的四行不必手工点，配套工程的\texttt{teaching-api/contract-check.mjs}把它们连同错误体形状写成了可执行检查：\texttt{node teaching-api/contract-check.mjs http://localhost:8080 --stage=lesson52}。同一个脚本把\texttt{--stage}换成\texttt{teaching}或\texttt{full}，就分别核对教学接口和接了数据库的完整后端。三种来源都全绿，才算真正做到“页面一行不改就能换数据源”；只要有一行红，说明契约在某一端被写歪了。
...
\paragraph{本节层次}核心：5.3.1、5.3.3；指导实践：5.3.2、5.3.4、5.3.6；拓展：5.3.5。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先完成5.2节，能从请求路径找到控制器方法及返回对象。
...
文档还应列出本地开发所需的端口、初始化命令和清理方式，保证测试结束后不会遗留后台进程。清单\ref{lst:ch05-http}给出本章工程的目录布局，可以对照它检查自己的工程：控制器、服务、领域与持久化各占一层，配置和迁移脚本单列。
...
配置应从代码中分离，并可被环境变量覆盖。密钥不得写入版本库。清单\ref{lst:ch05-application}的\texttt{application.yml}用\texttt{\$\{JWT\_SECRET\}}这类占位符引用环境变量，仓库里只留变量名和默认的非敏感值。
...
依赖注入优先使用构造器，使依赖明确并便于测试。控制器把DTO交给服务，不直接访问数据库。清单\ref{lst:ch05-query-controller}的控制器只有构造器注入的一个服务字段，测试时传入替身即可，不必启动数据库。
...
\paragraph{本节层次}核心：5.4.1、5.4.9、5.4.10；指导实践：5.4.2、5.4.4、5.4.5、5.4.6、5.4.13；拓展：5.4.3、5.4.7、5.4.8、5.4.11、5.4.12、5.4.14。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读5.3节核心内容与附录B P3，能解释表、主键、查询和事务。
...
实体映射的关键不是把每一列机械地搬到 Java 字段，而是明确身份、精度、生命周期和并发语义。水位属于有单位的测量值，使用\texttt{BigDecimal}并设置数据库精度，避免在累计统计时把二进制浮点误差当作工程变化；\texttt{occurredAt}记录设备采样时间，\texttt{createdAt}和\texttt{updatedAt}记录服务器侧生命周期。\texttt{version}由 JPA 在更新时递增，控制器将它映射为响应中的版本，服务层在替换前比较客户端版本。清单\ref{lst:ch05-jpa-entity}补齐了实体访问器，因而前面 DTO 的\texttt{ReadingResponse.from}能够通过公开的只读方法取值，而不会依赖反射或直接暴露字段。
...
审计字段至少分为创建时间、更新时间、操作者和来源。创建/更新时间可以由 Spring Data Auditing 自动填充，操作者来自经过认证的主体；设备上报的\texttt{assetId}和采样时间属于业务字段，不能用服务器时间覆盖。对逻辑删除记录保留删除时间和原因，查询默认过滤，审计查询按权限开放。
...
\]
计数和均值必须初始化，并在计算后再更新计数。清单\ref{lst:ch05-tx-service}按上式做增量更新，而不是把历史观测全部读进内存再求和——后者在观测量增长后会先变慢、再耗尽堆内存。
...
\paragraph{本节层次}核心：5.5.1；指导实践：5.5.2、5.5.3、5.5.5；拓展：5.5.4、5.5.6。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读5.2节与5.4节核心内容，区分输入校验、业务规则和数据库约束。


服务层按用例组织方法，例如“保存监测值”“确认告警”“查询测站详情”。输入DTO先通过Bean Validation完成格式校验，领域规则仍由服务或领域对象校验。统一响应不应把所有结果都包装成HTTP 200；HTTP状态码仍须表达协议层结果。清单\ref{lst:ch05-exception-handler}把校验失败翻译成 400 并指出出错字段，与表\ref{tab:api-contract}约定的错误体形状一致。
...
一个可交付的 REST 资源至少要说明集合查询、单项查询、创建、整体更新和删除的语义。案例平台对观测记录只允许“新增”和“订正为新版本”（见 8.1 节契约表），下面清单中的\texttt{DELETE}只用于讲解 REST 语义，真实平台不提供删除观测的接口。以监测读数为例，\texttt{GET /api/readings/{id}}返回单条记录，\texttt{GET /api/readings}支持分页与过滤，\texttt{POST /api/readings}创建新记录，\texttt{PUT /api/readings/{id}}替换可编辑字段，\texttt{DELETE /api/readings/{id}}删除尚未归档的记录。更新和删除前要检查资源版本、用户权限和业务状态；已进入审计归档的读数应采用逻辑失效或拒绝删除，而不是直接破坏时序证据。
...
清单\ref{lst:ch05-dto-mapper}把外部字段转换集中在映射边界，单位、质量码和版本号不会因为数据库字段改名而泄露到 API。\texttt{version}为后续的乐观锁校验预留位置；更新请求携带旧版本时，服务层可以在保存前检查版本是否仍然匹配，冲突返回409并提示客户端重新读取，而不是覆盖另一个值班员刚提交的数据。清单\ref{lst:ch05-command-service}把写入、订正与失效三种命令放在同一个应用服务里，可以看出它们共用同一道领域校验，区别只在状态迁移。
...
清单\ref{lst:ch05-problem-advice}只把字段名和稳定错误码放入响应，具体原因留在受控日志。四个处理器产出同一形状的\texttt{\{code, message, field?\}}，与表\ref{tab:api-contract}的通用约定、教学接口以及 5.2 节那个最小控制器完全一致——页面的错误分支因此不必区分数据来自哪一端。\texttt{field}指向第一个出错的字段，帮助前端定位输入框；对跨字段错误，服务层抛出带业务码的异常。异常处理器本身也应有测试，确保新异常不会意外回落到500，并验证响应的\texttt{Content-Type}为\texttt{application/problem+json}。
...
同一字段的错误要有稳定的错误码。\texttt{assetId}为空属于\texttt{FIELD\_REQUIRED}，测站不存在属于\texttt{STATION\_NOT\_FOUND}，水位超出批准范围属于\texttt{LEVEL\_OUT\_OF\_RANGE}，采样时间倒退属于\texttt{MEASURED\_AT\_ORDER}。错误码与 HTTP 状态码分离：字段错误和业务规则错误都可以返回400，但前端通过错误码决定提示文本和是否允许重试。错误详情不要包含数据库列名、SQL 或内部堆栈，以免把实现细节暴露给外部调用者。

校验分组适合处理创建和更新的差异。创建读数要求\texttt{assetId}、\texttt{occurredAt}、\texttt{value}和\texttt{quality}全部提供；更新读数可能只允许修改订正值和质量码，同时必须携带版本号；删除请求不需要读取体，但要检查资源状态和操作者权限。把三种请求复用为一个“万能 DTO”会产生大量可选字段，容易出现“字段缺失却被当作清空”的歧义。为每种用例定义独立 record，能让 OpenAPI 文档、测试和错误提示保持一致。
...
\paragraph{本节层次}指导实践：5.6.1、5.6.3、5.6.5；拓展：5.6.2、5.6.4、5.6.6、5.6.7、5.6.8。本节按所列层次学习；指导实践成果仍按章末要求验收。
...
JWT 密钥通过配置注入并以足够长度的 Base64 值提供。本章示例锁定 jjwt 0.11.x：\texttt{parseClaimsJws} 返回 Claims 之前已经完成签名验证，签名不合法会抛出异常，应用不能先手工解码 Base64 再“验证”字段。刷新流程若需要接受已过期的刷新令牌，只能在捕获\texttt{ExpiredJwtException}后读取其 Claims；由于解析器已经验证签名，随后还要检查\texttt{type=refresh}、issuer、audience、subject、jti 和服务端撤销状态，绝不能把过期 access token 当作刷新凭据。清单\ref{lst:ch05-jwt-parse}给出 jjwt 0.11.x 的正确调用顺序，注意签名验证发生在读取任何 Claim 之前。
...
清单\ref{lst:ch05-revocation}让撤销记录的 TTL 不超过原令牌剩余寿命，避免黑名单永久增长。高并发平台还可以使用用户令牌版本号：用户注销或改密时递增版本，过滤器比较令牌中的版本与当前账户版本；jti 黑名单适合撤销单个设备，版本号适合撤销用户全部会话，两者可以组合。撤销状态读取失败时要按安全策略拒绝请求或进入受限降级，不能把 Redis 不可达默认为“令牌有效”。过滤链解决“是谁”，方法级注解解决“能做什么”：清单\ref{lst:ch05-method-security}用\texttt{@PreAuthorize}把角色判断写在服务方法上，跨域策略集中在一处配置，不散落在各个控制器。
...
\paragraph{本节层次}核心：5.7.1；指导实践：5.7.2、5.7.3、5.7.4、5.7.5；拓展：5.7.6。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读5.4节事务基础与5.5节服务职责，能说明提交成功和处理成功的区别。


同一应用进程内的领域事件可降低模块耦合。若处理必须在数据库事务成功后执行，应使用\texttt{@TransactionalEventListener}并指定\texttt{AFTER\_COMMIT}，不能用普通\texttt{@EventListener}声称“提交后执行”。清单\ref{lst:ch05-cache-service}用这个注解在观测入库提交之后才更新指标，事务回滚时监听器不会被触发，指标也就不会记录一条并不存在的观测。
...
跨服务通信必须使用网络可达的消息基础设施。清单\ref{lst:ch05-metrics}的 Kafka 监听器位于告警服务，消费数据服务发布的事件；它不是进程内事件监听器，两者的区别在于消息跨进程后必须自己处理重复投递。
...
\section{性能与可观测性：往哪里找}\label{sec:ch05-perf-pointer}
...
\paragraph{进入本节所需知识}先完成5.2节接口验证；数据库和认证测试使用5.4、5.6节的实践骨架。
