...
\paragraph{运行与观察}在配套工程\texttt{backend}目录执行\texttt{mvn spring-boot:run}，控制台最后出现\texttt{Tomcat started on port 8080}。此时教学接口必须已经停掉，否则端口冲突（附录B P0）。浏览器打开\texttt{http://localhost:8080/api/assets}，看到四个对象的 JSON 数组；把第4章的 Vite 开发服务器指向 8080（它本来就指向 8080），刷新列表页，四个对象出现在页面上——页面代码没有改动，这就是契约的意义。表\ref{tab:ch05-first-run}是应当观察到的结果。
...
接口写完，验证不靠“浏览器能打开”，而靠第4章已经准备好的两样东西。第一，把第4章 S2 阶段页对着本工程重新走一遍表\ref{tab:ch04-stage-acceptance}的 S2 行：列表、详情、切换对象，页面表现应与对着教学接口时一致。第二，表\ref{tab:api-teach-faults}里教学接口用\texttt{teach=}注入的四种故障，现在要由真实行为触发：不带令牌访问（本节还没有认证，5.6 节补上后应得 401）、不存在的对象编码（404）、\texttt{from}晚于\texttt{to}（400 且\texttt{field}为\texttt{from}）、没有观测的对象（204）。表\ref{tab:ch05-first-verify}是核对单，四行全部与契约一致，本节完成。
...
\paragraph{本节层次}核心：5.3.1、5.3.3；指导实践：5.3.2、5.3.4、5.3.6；拓展：5.3.5。核心路线只要求读完核心小节。
...
\paragraph{本节层次}核心：5.4.1、5.4.9、5.4.10；指导实践：5.4.2、5.4.4、5.4.5、5.4.6、5.4.13；拓展：5.4.3、5.4.7、5.4.8、5.4.11、5.4.12、5.4.14。核心路线只要求读完核心小节。
...
\paragraph{本节层次}核心：5.5.1；指导实践：5.5.2、5.5.3、5.5.5；拓展：5.5.4、5.5.6。核心路线只要求读完核心小节。
...
\paragraph{本节层次}指导实践：5.6.1、5.6.3、5.6.5；拓展：5.6.2、5.6.4、5.6.6、5.6.7、5.6.8。本节没有核心小节，课堂核心路线可整体跳过。
...
\paragraph{本节层次}核心：5.7.1；指导实践：5.7.2、5.7.3、5.7.4、5.7.5；拓展：5.7.6。核心路线只要求读完核心小节。
