...
系统停止使用时，需要迁移业务、归档或销毁数据、撤销账号和密钥、保留审计证据，并确认下游系统不再依赖旧接口。退役是受控活动，不是简单关闭服务器。

本段涉及的字段、职责或条件见表\ref{tab:sdlc-deliverables}，读者可据此核对取值范围和验收依据。
本段涉及的字段、职责或条件见表\ref{tab:lifecycle-comparison}，读者可据此核对取值范围和验收依据。
本段涉及的字段、职责或条件见表\ref{tab:moscow-example}，读者可据此核对取值范围和验收依据。
本段涉及的字段、职责或条件见表\ref{tab:requirement-quality-check}，读者可据此核对取值范围和验收依据。
本段涉及的字段、职责或条件见表\ref{tab:water-plan-decision}，读者可据此核对取值范围和验收依据。
本段涉及的字段、职责或条件见表\ref{tab:requirements-traceability}，读者可据此核对取值范围和验收依据。
\begin{table}[htbp]
\centering
...
瀑布模型按需求、设计、实现、验证和运行依次推进，阶段评审和文档边界清楚。它适用于需求相对稳定、合规审查严格、软硬件接口明确的部分，但反馈较晚，早期误解可能在后期放大。

本段所述流程或结构用图\ref{fig:waterfall-model}具体呈现，读者可沿图中的节点和箭头检查职责边界。
本段所述流程或结构用图\ref{fig:prototype-model}具体呈现，读者可沿图中的节点和箭头检查职责边界。
本段所述流程或结构用图\ref{fig:incremental-model}具体呈现，读者可沿图中的节点和箭头检查职责边界。
本段所述流程或结构用图\ref{fig:order-process-flowchart}具体呈现，读者可沿图中的节点和箭头检查职责边界。
本段所述流程或结构用图\ref{fig:dfd-level-zero}具体呈现，读者可沿图中的节点和箭头检查职责边界。
本段所述流程或结构用图\ref{fig:dfd-level-one}具体呈现，读者可沿图中的节点和箭头检查职责边界。
\begin{figure}[H]
\centering
...
原型用于澄清用户难以直接表达的交互、报表或空间展示需求。团队快速构造可评估版本，收集反馈后修订需求。原型必须注明是探索性还是演化性；探索性原型不应在缺少架构、安全和测试的情况下直接上线。
...
\textit{图中把本段的流程、架构或状态关系具体化，读者可结合图\ref{fig:prototype-model}核对前文的节点与边界。}
\end{figure}

\subsection{迭代与增量模型}

增量模型把能力分成若干可运行交付，例如先交付测点查询，再交付质量标记、预警和工单。迭代模型允许同一能力反复改进。两者常结合使用：每个增量经过需求、设计、实现和验证，小步获得真实反馈。
...
\textit{图中把本段的流程、架构或状态关系具体化，读者可结合图\ref{fig:incremental-model}核对前文的节点与边界。}
\end{figure}
...
\textit{表中把本段的字段、条件或职责具体化，读者可结合表\ref{tab:lifecycle-comparison}核对取值与验收口径。}
...
MoSCoW方法把需求分为Must、Should、Could和本次Won't。Must表示没有它就无法验收、无法合规或无法安全运行；Should具有高价值但存在可接受替代；Could在时间允许时实现；Won't表示明确不进入当前范围。优先级必须记录理由、依赖和批准人，并在迭代或变更时复核。

\begin{table}[htbp]
\centering
\caption{清源灌区需求优先级示例}
\label{tab:moscow-example}
\textit{表中把本段的字段、条件或职责具体化，读者可结合表\ref{tab:moscow-example}核对取值与验收口径。}
...
团队通过评审、原型、模型走查和验收用例验证需求。冲突由业务价值、安全、合规、成本和依赖共同决策，而不是由声音最大的人决定。变更后更新需求版本、影响分析和批准记录。

\begin{table}[htbp]
\centering
\caption{需求质量检查清单}
\label{tab:requirement-quality-check}
\textit{表中把本段的字段、条件或职责具体化，读者可结合表\ref{tab:requirement-quality-check}核对取值与验收口径。}
...
本段流程图符号的线型、箭头和终点语义见表\ref{tab:system-flow-symbols}，可用来核对图形化需求说明。
...
\textit{图中把本段的流程、架构或状态关系具体化，读者可结合图\ref{fig:order-process-flowchart}核对前文的节点与边界。}
\end{figure}

\subsection{数据流图}

数据流图（DFD）只表达数据如何在外部实体、处理和数据存储之间流动。上下文图把系统视为一个处理，先确定边界；再逐层分解，但父图输入输出必须与子图保持平衡。
...
\textit{图中把本段的流程、架构或状态关系具体化，读者可结合图\ref{fig:dfd-level-zero}核对前文的节点与边界。}
\end{figure}
...
\textit{图中把本段的流程、架构或状态关系具体化，读者可结合图\ref{fig:dfd-level-one}核对前文的节点与边界。}
\end{figure}

\subsection{数据字典}

数据字典定义数据流、数据存储和数据元素的结构与约束。每个字段的说明栏应给出编码格式、数据类型、单位、来源、取值域和校验规则，例如计划编号采用项目编码加年度序号，状态字段只能取受控枚举，流量字段注明精度与上限。清晰的字段契约能让接口、数据库和测试用例使用同一口径。

计划编号、时间、流量和状态字段的取值域见表\ref{tab:water-plan-dictionary}，它是接口校验与数据库建模的共同契约。
...
当业务结果由多个条件组合决定时，决策表比长段落更清楚。下表示例只用于需求澄清，真实阈值应由批准规则确定。

\begin{table}[htbp]
\centering
\caption{用水计划审批决策表示例}
\label{tab:water-plan-decision}
\textit{表中把本段的字段、条件或职责具体化，读者可结合表\ref{tab:water-plan-decision}核对取值与验收口径。}
...
\textit{表中把本段的字段、条件或职责具体化，读者可结合表\ref{tab:requirements-traceability}核对取值与验收口径。}
