...
\paragraph{本节层次}核心：3.1.2；指导实践：3.1.1、3.1.3。核心路线只要求读完核心小节。
...
\paragraph{本节层次}核心：3.2.1、3.2.2、3.2.3；指导实践：3.2.4。核心路线只要求读完核心小节。
...
控制器只负责协议转换和参数校验，不把预警规则写在 Web 层。代码清单\ref{lst:ch03-controller}以 Spring MVC 的构造器注入保持依赖显式；它暴露的路径就是表\ref{tab:api-contract}中的“最新观测”端点，第4章的页面和第5章的实现都对着同一条路径。
...
定义\texttt{MonitoringAcquisitionService}接口，暴露\texttt{pauseAcquisition(assetId, reason, until)}和\texttt{resumeAcquisition(assetId)}方法，隐藏设备协议、采样调度和断线重传逻辑。暂停只用于设备检修等受控场景，必须校验授权、记录原因和到期时间，并自动恢复单个测点采集，不中断整站运行。
...
\paragraph{本节层次}核心：3.3.1、3.3.2、3.3.5；拓展：3.3.3、3.3.4、3.3.6、3.3.7。核心路线只要求读完核心小节。
...
\paragraph{本节层次}核心：3.4.1；指导实践：3.4.2、3.4.3。核心路线只要求读完核心小节。
...
\paragraph{本节层次}核心：3.5.1；指导实践：3.5.2、3.5.3。核心路线只要求读完核心小节。
...
\paragraph{本节层次}拓展：3.6.1、3.6.2、3.6.3。本节没有核心小节，课堂核心路线可整体跳过。
