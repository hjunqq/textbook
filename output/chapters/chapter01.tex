...
\begin{warningbox}
\textbf{常见概念误用：防洪标准与洪灾风险。}防洪标准的“30年一遇”或“100年一遇”描述的是工程针对致灾因子所采用的重现期目标；建设方案因此应写成“防洪标准由30年一遇提高到100年一遇”。洪灾风险则是致灾因子、暴露度、脆弱性和应对能力共同作用的结果，还会受到调度能力、预警提前量以及极端事件尾部影响。风险评估需要同时说明危险性、承灾体和防灾减灾能力，不能把一个重现期数字直接当作风险下降幅度。
\end{warningbox}

\subsection{政策主线与建设方向}

% TODO-核对[O8-03]：smartWaterGuidance2021 条目同时指向《关于大力推进智慧水利建设的指导意见》和《“十四五”期间推进智慧水利建设实施方案》两份不同文件，且 URL 指向贵州省水利厅转载页而非水利部官网；需拆分条目并换官方 URL —— 待编者确认后修改，勿自行改写
2019年前后的“补短板、强监管”是水利改革发展的阶段性总基调；进入“十四五”时期，水利部在新的政策文件中把智慧水利建设纳入推动水利高质量发展的实施路径，提出“需求牵引、应用至上、数字赋能、提升能力”，并以数字孪生流域为核心建设“四预”能力\cite{smartWaterGuidance2021}。阅读政策时应按发布年份、文件标题和适用范围分别建立时间线。

% TODO-核对[O8-02]：“七大江河数字孪生流域”的数量与措辞出处 —— 待编者确认后修改，勿自行改写
《“十四五”期间推进智慧水利建设实施方案》提出建设七大江河数字孪生流域\cite{smartWaterGuidance2021}。随后，国家水网建设与数字孪生水利工程相关文件进一步把流域、水网和工程联系起来\cite{nationalWaterNetwork2023,twinProject2024}。平台论证应把每项成效绑定到可复核的指标、数据来源和计算口径，例如分别报告预警提前量、数据完整率、模型误差和处置闭环时延，并注明统计周期、样本范围与对照基线。这样形成的“业务需求—数据底板—模型平台—应用闭环”主线，既便于验收，也便于学生复现实验。

政策目标不等于项目验收指标。项目仍需把“四预”分解为可测试问题，例如：输入场景是否完整、模型是否可复现、预警是否按权限流转、预演是否保留方案版本、预案是否能关联责任与资源。

\subsection{感知、通信与数据交换}

监测链路通常包含传感器、遥测终端或边缘网关、通信网络、接入服务和数据平台。不同层使用的规范不可混为一谈，表\ref{tab:monitoring-protocol-roles}按“各自解决什么问题”把三类规范分开：通信规约约束测站与中心站之间报文如何编码、重传和应答，消息协议约束设备或网关的消息在网络中如何承载与订阅，数据交换标准约束一段观测时间序列如何被另一个系统正确读懂。把WaterML~2.0当作测站上报协议，或用MQTT去替代SL/T~651承担的规约职责，都是把不同层的问题混在了一起。
...
类别 & 例子 & 用途 \\
\midrule
% TODO-核对[O8-04]：SL/T 651—2014 的标准全名是否为《水文监测数据通信规约》；“智能传感器与遥测终端之间”是否属于该标准适用范围；SL 与 SL/T 前缀变更宜加脚注 —— 待编者确认后修改，勿自行改写
...
汛期某时刻，库区雨量站上报分钟雨量。遥测终端按通信规约编码，接入服务校验站码、时间和单位；质量服务标记缺测、突变或回补；洪水预报模型读取同一输入快照并输出过程线；规则服务结合批准阈值生成黄色预警。值班员确认后创建工单，专业人员复核模型与现场情况，审批人决定是否启动预案，处置结果再与后续实况比较。图\ref{fig:flood-warning-loop}把这条链路画成闭环：上半行是数据从监测、接入校验、质量标记到模型计算与预警生成的路径，下半行是人的确认、会商、处置与复核，虚线表示复核结论反过来校正质量规则与模型参数。读图时要留意，链路既没有在“预警发出”处结束，也没有在“处置完成”处结束。
...
\paragraph{本章图表导读}
表\ref{tab:smart-water-features}把业务牵引、数据可信、模型支撑、协同闭环和安全运行放在同一张验收视图中。
表\ref{tab:monitoring-protocol-roles}用于区分测站通信规约、通用消息协议和数据交换标准的职责。
图\ref{fig:flood-warning-loop}沿一次洪水预警链路展示数据质检、模型计算、人工确认和复核归档的顺序。
图\ref{fig:smart-water-architecture}说明四层架构中的观测上行与受控下行；图\ref{fig:sdlc-flow}则把需求、设计、实现、验证、运行和反馈组成可追踪的生命周期闭环。阅读这些图表时，应同时标注责任角色、输入证据和验收条件。
