...
经典OGC服务常用KVP（Key-Value Pair）请求表达操作。下面三条最小请求分别展示图像、要素和覆盖数据的返回类型；实际项目还需加入认证、版本、异常格式和服务地址。WMS 1.3.0使用\texttt{CRS}参数而不是旧版本的\texttt{SRS}；当坐标系为EPSG:4490时，规范轴序是纬度在前、经度在后，\texttt{BBOX}应按该顺序填写。客户端若沿用“经度,纬度”的习惯，会得到空白图或位置偏移。
...
注意轴序：EPSG:4490 的官方定义是纬度在前，但 GeoServer 对简写\texttt{EPSG:4490}形式的\texttt{SRSNAME}默认按经度在前处理，本例即按此约定书写；若改用\texttt{urn:ogc:def:crs:EPSG::4490}形式，BBOX 必须交换为纬度在前，这是 WFS 联调中最常见的“查不到要素”原因。
...
下面示例把CGCS2000经纬度转换到中央经线111度的6度带（19带），采用无前缀东坐标。\texttt{tmerc}表示横轴墨卡托/高斯—克吕格计算；GRS80与CGCS2000椭球参数一致，\texttt{+k\_0}是PROJ 6+推荐写法，\texttt{+no\_defs +type=crs}用于明确坐标定义。带号写入元数据，交付系统若要求带号前缀，再按项目规范转换。
