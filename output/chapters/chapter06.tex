...
图形管线的每一级都应有可观测的输入和输出。模型矩阵错误通常表现为整体位置或朝向异常，观察矩阵错误表现为相机方向和“上下颠倒”，投影矩阵错误则表现为远近比例、裁剪或深度顺序异常。把问题定位到管线阶段，比盲目调节模型坐标更可靠。图\ref{fig:ch06-render-pipeline}以同一橙色顶点展示构件、相机视锥与屏幕像素的关系，底部列出坐标变换与裁剪步骤。
...
图\ref{fig:ch06-bound-scene}展示S4阶段页的简化坝体与当前视野内的测点。场景中的对象数量、当前可见数量和成功绑定数量应分别核对；相机视野外的测点仍可能已经正确载入。

\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{images/runtime/s4-scene.png}
\caption{简化坝体与测点的场景绑定（S4阶段页实际运行截图）}
...
图\ref{fig:ch06-coordinate-chain}把“经度—分带—投影—场景原点”的关系画成一条可审计链。任何一步缺少带宽或中央经线，后续三维定位都只能算近似结果。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[node distance=8mm, every node/.style={font=\small},
  box/.style={draw,rounded corners,align=center,minimum width=2.6cm,minimum height=9mm,fill=orange!8},
  arr/.style={-{Latex[length=2mm]},thick}]
  \node[box] (geo) {CGCS2000经纬度\\$\lambda,\varphi$};
  \node[box,right=of geo] (zone) {3度/6度分带\\$n,N,\lambda_0$};
  \node[box,right=of zone] (proj) {横轴墨卡托\\$E,N,H$};
  \node[box,right=of proj] (local) {工程局部原点\\三维场景};
  \draw[arr] (geo)--node[above]{选带}(zone);
  \draw[arr] (zone)--node[above]{投影}(proj);
  \draw[arr] (proj)--node[above]{平移/旋转}(local);
\end{tikzpicture}
...
图\ref{fig:ch06-aerial-control-network}给出了控制网络的教学化布设：连接点负责把相邻影像锁定到同一块地物，像控点把模型约束到工程坐标，检查点保持独立用于验收。控制点应覆盖坝轴线两端、坝顶与坝脚、高程变化明显的坡面、闸室和场景边界；检查点宜与像控点分区布设，避免所有点落在同一小区域。图中不同符号区分点的用途；平差文件还需逐点记录角色、测量方法、坐标基准、精度等级和是否参与平差。

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[>=Stealth,node distance=1.4cm and 1.5cm]
\tikzset{flight/.style={draw=Primary,rounded corners,minimum width=8.2cm,minimum height=.42cm,fill=PrimaryLight},
pt/.style={circle,draw=Accent,fill=Accent!20,minimum size=5mm,inner sep=0pt},
cp/.style={rectangle,draw=TipColor,fill=TipColor!15,minimum size=5mm,inner sep=0pt},
ck/.style={diamond,draw=WarningColor,fill=WarningColor!20,minimum size=5mm,inner sep=0pt}}
\node[flight] (f1) {航带A：\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;};
\node[flight,below=of f1] (f2) {航带B：\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;};
\node[flight,below=of f2] (f3) {航带C：\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;};
\node[pt,above left=0.08cm and 1.0cm of f1] (p1) {};
\node[pt,above right=0.08cm and 1.0cm of f1] (p2) {};
\node[cp,left=0.3cm of f2] (c1) {};
\node[cp,right=0.3cm of f2] (c2) {};
\node[ck,left=0.3cm of f3] (k1) {};
\node[ck,right=0.3cm of f3] (k2) {};
\draw[<->,draw=Accent,thick] (p1)--(p2);
\draw[<->,draw=TipColor,thick] (c1)--(c2);
\draw[<->,draw=WarningColor,thick] (k1)--(k2);
\node[below=0.55cm of f3,align=left] {圆点：连接点\quad 方框：像控点\quad 菱形：独立检查点};
\end{tikzpicture}
\caption{倾斜摄影空三控制与独立检查点布设示意}
...
\]
其中$PE$为物理实体、$VE$为虚拟实体、$Ss$为服务、$DD$为孪生数据、$CN$为连接。孪生数据 $DD$ 包括观测、状态和模型运行所需的数据；各维度在水利平台中的对应关系如下图所示。图\ref{fig:dt-five-dimensions}把五个维度画在一起，并用连接维$CN$把其余四维两两串起来。连接维承载观测更新与执行反馈；双向连接中断时，虚拟状态可能落后于现场，指令结果也无法得到确认。
...
平台可划分为物理对象层、感知连接层、数据底板层、模型与知识层、服务层和交互应用层。各层分别承担数据校验、模型运行、结果解释与指令审批等职责。图\ref{fig:dt-layered-architecture}把这六层自下而上排开，并在层间标出数据上行与指令下行两条通道。层间接口规定输入、输出和失败响应；保持接口兼容时，可替换一层的实现而继续使用相邻层。
...
一次完整闭环包括“感知—质检—状态更新—模型计算—风险解释—人工确认—指令下发—执行反馈—效果评估”。并非所有场景都允许自动控制：大坝安全和防洪调度通常需要权限、会商和人工确认。图\ref{fig:dt-closed-loop}把这些环节连成回路，并把需要人工确认的位置单独标出。指令下发后还需核对执行回执，并用后续观测评估效果；缺少任一项时，只能记录已完成的步骤，无法确认执行结果。
...
\node[svc,below=of gateway] (cmd) {命令下发\\审批+幂等键};
\node[svc,right=of cmd] (device) {设备执行\\回执状态};
...
\draw[->,draw=Accent] (cmd)--node[above,font=\small]{受控命令}(device);
\draw[->,draw=Accent] (device)--(audit);
\draw[->,draw=Accent] (cmd)--(audit);
