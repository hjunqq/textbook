...
本节给出模型卡和可信度管理的方法框架；清源水库案例中的场景版本、不确定性和方案比较见\ref{sec:ch08-twin-model-card}节。两处内容分别承担“通用方法”和“工程实现”的职责，模型卡字段、输入快照和审批证据应保持同一语义。
...
本节给出状态事件、查询接口和控制命令的通用契约；清源水库案例中的模型任务编排、运行生命周期和消息落地见\ref{sec:ch08-twin-contract}节。框架层关注字段语义与幂等边界，案例层负责把它们绑定到具体服务、数据库事务和审计日志。
...
本节给出数据、空间、模型、服务、闭环和运维六类验收框架；清源水库案例的失效降级、验收脚本和阶段门禁见\ref{sec:ch08-twin-acceptance}节。验收证据应从通用指标回到具体输入、版本、责任人和复核结果。
