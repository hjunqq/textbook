...
\label{lst:ch06-wms-getmap}
\begin{lstlisting}[language=HTTP,breaklines=true,caption={WMS 1.3.0 GetMap最小请求}]
...
\label{lst:ch06-wfs-getfeature}
\begin{lstlisting}[language=HTTP,breaklines=true,caption={WFS GetFeature最小请求}]
...
\label{lst:ch06-wcs-getcoverage}
\begin{lstlisting}[language=HTTP,breaklines=true,caption={WCS GetCoverage最小请求}]
...
\begin{lstlisting}[language=JavaScript,breaklines=true,caption={Cesium无商业底图密钥的WMTS初始化}]
...
实际部署需要将示例域名替换为课程或项目服务器，并在服务端配置跨域、访问控制和缓存策略。Cesium示例关闭了Ion底图、地形、地理编码、时间轴和动画组件，因此不会隐式请求商业token；业务底图只来自自建GeoServer。由于EPSG:4490是地理坐标瓦片矩阵，必须使用\texttt{GeographicTilingScheme}，并把\texttt{maximumLevel}设置为GWC gridset实际提供的最大层级。GeoServer GWC默认内置EPSG:4326与EPSG:900913两个gridset，EPSG:4490需要手工创建并核对原点、瓦片宽高、比例尺和矩阵层级。
...
\label{lst:ch06-geoserver-gridset}
\begin{lstlisting}[language=YAML,breaklines=true,caption={GeoServer GWC EPSG:4490 gridset配置要点}]
gridSet:
  name: EPSG:4490
  srs: 4490
  tileWidth: 256
  ...
  levels:
    - {level: 0, resolution: 0.703125}
    - {level: 1, resolution: 0.3515625}
  cacheLayers:
...
\label{lst:ch06-zone-calc}
\begin{lstlisting}[language=JavaScript,caption={3度带与6度带带号计算}]
...
高程异常具有明显的区域性。我国大陆范围内可见约$-40\,\mathrm{m}$至$+70\,\mathrm{m}$的量级变化；东部平原通常在约$\pm10\,\mathrm{m}$的范围内变化，西部高原的变化可达数十米量级。这里的数值用于帮助学生建立数量级概念，工程计算应采用测区批准的似大地水准面或高程异常模型，例如CNGG2011或省级加密模型，并在元数据中记录模型名称、版本、格网分辨率、适用范围和发布日期。模型文件随成果归档，程序不得用固定常数替代区域模型。

清单\ref{lst:ch06-height-convert}把高程转换写成显式函数，输入值和输出值都保留单位与基准标签；当缺少异常模型时，程序应拒绝静默换算。

\label{lst:ch06-height-convert}
\begin{lstlisting}[language=JavaScript,caption={椭球高到正常高与正高的转换}]
...
\label{lst:ch06-coordinate-validate}
\begin{lstlisting}[language=JavaScript,caption={坐标元数据校验与轴序显式化}]
...
\label{lst:ch06-local-origin}
\begin{lstlisting}[language=JavaScript,caption={工程坐标与三维局部原点互转}]
...
生产流程包括航线设计、影像采集、质量检查、空中三角测量、密集匹配、网格与纹理生成、坐标校核、切片和发布。Agisoft Metashape是PhotoScan更名后的现行名称；Bentley ContextCapture现已并入iTwin Capture Modeler产品线。项目文档应记录软件版本、产品名称、处理参数和名称沿革，便于学生按官方资料复现实验，也便于成果在多年运行后追溯。

倾斜摄影的精度不是软件“自动生成”的属性，而是由任务设计、传感器标定、外业控制和内业检核共同决定。任务书首先写明成果用途、比例尺、覆盖范围、坐标和高程基准，再反推地面采样距离（GSD）、影像重叠度、飞行高度、相机曝光参数和控制点布设。以1:500地形图为例，GSD通常把目标设在不大于3\,cm的量级；这是任务设计的示例目标，平面和高程限差仍须按适用规范和项目合同查表确认（TODO-核对）。CH/Z 3004、CH/Z 3005分别约束低空数字航空摄影测量的外业与内业环节，GB/T 18314用于卫星定位测量成果的技术依据，三者应与项目适用的水利行业规范共同列入质量计划\cite{chz3004,chz3005,gbt18314}。
...
成果精度报告应按平面和高程分别给出统计量。现有式(\ref{eq:rmse-x})和式(\ref{eq:rmse-y})分别描述东、北分量，工程验收通常还需要平面点位均方根误差$ m_p $，其定义为
\begin{equation}
...
\midrule
1:500地形图示例 & GSD目标通常不大于3\,cm & 平面使用$m_p$，高程使用$m_h$；限差按适用条款查表 & 设计目标示例，具体限值 TODO-核对 \\
平面检查点 & 点号、坐标基准、$\Delta X$、$\Delta Y$ & 报告$m_x$、$m_y$、$m_p$及最大绝对误差 & 不把分量误差替代平面点位指标 \\
高程检查点 & 高程基准、$\Delta H$、测量方法 & 报告$m_h$及最大绝对误差 & 与平面统计分开，记录模型版本 \\
异常点处理 & 残差、复核状态、处置人 & $|r|>2\,\mathrm{MSE}$进入复核；$|r|>3\,\mathrm{MSE}$判为粗差候选 & 删除或剔除必须记录原因和证据 \\
\bottomrule
\end{tabular}
\end{table}

粗差处理遵循“先复核、后判定、可追溯”的顺序。把残差绝对值超过2倍均方误差（MSE）的点列入复核清单，检查点位识别、坐标录入、时间同步和影像质量；超过3倍MSE的点可判为粗差候选，但仍需由测绘人员确认并记录剔除原因、原始残差、复核证据和重新计算结果。报告中应同时保留剔除前后的统计量，避免通过删除不利点“优化”数字。若多个异常点沿同一航带成带状分布，应优先调查系统误差；若单点异常且现场记录完整，才可能是点位识别或录入错误。

下面的独立计算逻辑把分量误差、合成平面误差和粗差标记同时输出，学生可以用同一份检查点 CSV 复核软件报告。代码中的阈值只负责生成复核队列，最终限差仍由项目适用规范和验收文件决定。

\label{lst:ch06-accuracy-check}
\begin{lstlisting}[language=JavaScript,caption={检查点平面与高程精度计算及粗差复核}]
function evaluateCheckPoints(points, mseThreshold) {
  ...
  const review = points.map((p) => {
    ...
    const flag = residual > 3 * mseThreshold ? 'gross-error-candidate'
      : residual > 2 * mseThreshold ? 'review' : 'pass';
...
在成果交付单中，平面限差、高程限差、MSE计算口径和粗差处置规则应作为一个整体签字确认。若项目采用地方细化规范或合同中的更严要求，应在表\ref{tab:ch06-aerial-accuracy}对应行补充版本、条款和适用范围；若规范版本发生变化，重新计算检查点并保留旧版报告，确保三维模型、BIM构件和监测平台引用的是同一成果版本。
...
最终的架构验收以业务证据为中心组织。抽取一条观测事件，验证它能从接入日志追到质量结论、状态版本、模型输入、风险结果、人工确认、指令回执和复盘记录；再制造一条重复消息、一个单位错误、一次模型超时和一次无权限命令，验证系统分别给出幂等忽略、可疑标记、降级结果和拒绝审计。正常流证明系统能工作，失败流证明系统知道何时停止、如何恢复以及由谁负责。第8章的案例实现沿用这套证据链，读者可在\ref{sec:ch08-twin-acceptance}看到端到端脚本和阶段门禁的具体写法。
...
复核记录同时写出模型版本、数据窗口和异常处置人。
并注明复核时间与责任角色。
若责任角色未签名，验收状态保持“待复核”，不进入正式发布。
状态变更、签名和发布动作必须使用同一追踪号关联。
只有证据链完整，系统结果才可进入正式业务处置。
并由审计角色独立复核。
...
本节给出模型卡和可信度管理的方法框架；清源水库案例中的场景版本、不确定性和方案比较见第8章\ref{sec:ch08-twin-model-card}。两处内容分别承担“通用方法”和“工程实现”的职责，模型卡字段、输入快照和审批证据应保持同一语义。
...
本节给出事件时间和质量码的通用处理框架；案例中对象编码、时空基线与孪生状态的落地见第8章\ref{sec:ch08-twin-event-quality}。读者应先掌握事件时间、处理时间和质量优先级，再阅读案例的表结构与回放流程。
...
三维交互的详细视觉编码、LOD降级、键盘可达和图表联动由第7章负责，本节只保留跨层证据链的接口约束；具体展示实现可参见第7章\ref{sec:monitoring-interaction}。第6章的职责是定义对象、状态、模型证据和处置记录如何连接，第7章的职责是把这些证据转成可读、可操作的二维与三维界面。
...
本节给出状态事件、查询接口和控制命令的通用契约；清源水库案例中的模型任务编排、运行生命周期和消息落地见第8章\ref{sec:ch08-twin-contract}。框架层关注字段语义与幂等边界，案例层负责把它们绑定到具体服务、数据库事务和审计日志。
...
\begin{lstlisting}[language=JSON,caption={模型事件契约示例}]
{
  "eventId": "01J5...",
...
本节给出安全边界、职责分离和降级原则；案例中底板与算法解耦、权限范围和模拟标注的实现见第8章\ref{sec:ch08-twin-permission}。无论使用何种产品，权限判断都必须在后端完成，高风险建议必须保留审批与回滚证据。
...
本节给出服务、数据、模型和业务闭环的可观测框架；案例中的备份、恢复、值守时间线和复盘记录见第8章\ref{sec:ch08-twin-lineage}。框架要求指标能够回答“是否可用、是否可信、是否已处置”，案例则把问题落实为看板、日志和演练脚本。
...
本节给出数据、空间、模型、服务、闭环和运维六类验收框架；清源水库案例的失效降级、验收脚本和阶段门禁见第8章\ref{sec:ch08-twin-acceptance}。验收证据应从通用指标回到具体输入、版本、责任人和复核结果。
...
本节给出课程项目的四次迭代路线；案例中的端到端实现切片、验收脚本和分阶段交付见第8章\ref{sec:ch08-twin-course}。学生先完成对象和坐标契约，再做场景绑定、模型服务和业务闭环，每一轮都提交可复现证据。
...
\item 使用5个以上独立检查点计算$m_x$、$m_y$、$m_p$和$m_h$，依据适用规范给出的平面点位与高程限差判断成果是否合格；对绝对残差超过$2\,\mathrm{MSE}$的点建立复核记录，对超过$3\,\mathrm{MSE}$的粗差候选说明复核、剔除理由及前后统计量。
