...
\item 区分WMS、WMTS、WFS与WCS，正确处理CGCS2000、投影坐标和高程基准；
...
\begin{tipbox}
\paragraph{工程版本线：v2（认证与观测API） $\rightarrow$ v3（三维场景接入）}起点是已完成认证和观测查询的 v2 前后端。本章结束时你应交付 \textbf{v3}：在监测页之外新增一个三维场景页，加载工程模型并把测点定位到 CGCS2000 坐标——三维部分作为独立页面接入 v2 的路由与认证，不改动既有接口；本章清单中的坐标转换与模型加载代码即是该页面的实现素材。
\end{tipbox}

\section{WebGL2与Three.js三维渲染基础}

\paragraph{本节层次}核心：6.1.1、6.1.2、6.1.4、6.1.5；拓展：6.1.3。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读4.4节的模块与事件；使用4.5节请求骨架取得对象列表，准备现代浏览器。


\subsection{从一个几何体开始：场景、相机、渲染器与循环}

\paragraph{业务问题}第7章要把 28 个测点画进坝体三维场景，点一下测点弹出它的观测曲线。在加载任何工程模型之前，先用一个长方体代替坝体，把 Three.js 的四个基本对象和一个渲染循环跑通；本小节结束时你有一个能转动的“坝体”，并且知道黑屏时先查哪三处。

\paragraph{四个对象}场景（\texttt{Scene}）是所有物体的容器；相机（\texttt{PerspectiveCamera}）决定从哪里、以多大视角看；渲染器（\texttt{WebGLRenderer}）把场景和相机变成画布上的像素；网格（\texttt{Mesh}）= 几何体 + 材质，用于表示可渲染的物体。渲染时，相机确定视点与可见范围，渲染器据此绘制场景中的网格。清单\ref{lst:ch06-first-scene}是完整可运行的一页脚本，放进第4章的 Vite 工程即可（\texttt{npm install three@0.160}），尺寸单位与 8.1 节参数一致：坝高\cpDamHeight\,m，坝顶高程\cpCrestElev\,m，因此长方体的底面放在 \cpBaseElev\,m。
...
\paragraph{可观察结果}页面出现灰蓝色长方体，拖动鼠标可旋转视角，滚轮可缩放。在脚本末尾加入\texttt{console.log(dam.position.y)}，控制台输出146；模块变量需在模块内读取。核对裁剪时，先执行\texttt{camera.updateMatrixWorld()}，再打印\texttt{dam.getWorldPosition(new THREE.Vector3()).}\allowbreak\texttt{applyMatrix4(camera.matrixWorldInverse).z}。初始观察坐标$z\approx-331.35$，中心深度$-z$位于1至2000之间；物体可见性还需结合视场与包围体判断。

\paragraph{黑屏时先查哪三处}表\ref{tab:ch06-first-scene-faults}把初学者最常见的三种黑屏按“现象—原因—验证”列出。相机设置、材质与光照、画布与渲染循环分别对应三类排查。即使控制台没有异常，也要检查物体是否处于视锥内、材质是否有可用光照，以及画布是否实际绘制。
...
\paragraph{自测}（1）把坝体改为 8.1 节参数表中的“\cpGateCount 孔弧形闸门”：在坝顶加 \cpGateCount 个宽 \cpGateWidth\,m 的小长方体，底面在闸底高程 \cpGateSillElev\,m，说出每个的 \texttt{position.y}。（2）把远裁剪面改成300并更新投影矩阵，哪些部分会被裁去？（初始坝体跨越远裁剪面，较远部分被裁去；中心在裁剪范围外时，近侧仍可能可见。）（3）\texttt{setAnimationLoop}相比手写\texttt{requestAnimationFrame}循环有什么作用？（它统一管理普通渲染与WebXR循环；常规页面下两种方式通常都会在后台暂停。离开场景时仍需用\texttt{setAnimationLoop(null)}停止循环并释放资源。）
...
\[
  \mathbf{p}_{\mathrm{clip}}=PVM\,\mathbf{p}_{\mathrm{model}}.
\]
...
示例使用WebGL2，初始化时先检查浏览器是否支持所需能力。WebGL1对顶点数组对象、32位索引和扩展支持的要求不同，若确需兼容，应单独实现并测试兼容分支。清单\ref{lst:ch06-webgl2-context}取上下文并当场检测能力：拿不到\texttt{webgl2}就明确报错退出，避免后续代码访问空上下文，使错误能定位到初始化阶段。
...
顶点着色器必须显式声明使用的uniform。清单\ref{lst:ch06-glsl-vertex}把模型、观察和投影三个矩阵分开传入而不是预乘成一个，物体位置不对时可以逐个矩阵替换成单位阵，定位到底是哪一步的坐标错了。
...
图形管线的每一级都应有可观测的输入和输出。模型矩阵错误通常表现为整体位置或朝向异常，观察矩阵错误表现为相机方向和“上下颠倒”，投影矩阵错误则表现为远近比例、裁剪或深度顺序异常。把问题定位到管线阶段，比盲目调节模型坐标更可靠。图\ref{fig:ch06-render-pipeline}以同一橙色顶点展示构件、相机视锥与屏幕像素的关系，底部列出坐标变换与裁剪步骤；视口变换得到连续窗口坐标，后续光栅化再生成片元。
...
图\ref{fig:ch06-render-pipeline}中的近、远裁剪面限定沿观察方向的深度范围。调试时可输出构件包围盒和观察坐标，检查顶点是否越过裁剪面；物体被裁掉时，即使模型已加载成功，画布上也可能看不到它。

持续更新的 Three.js r160+ 场景需要场景对象、相机、渲染器和渲染循环。只有把网格加入场景而没有渲染器，浏览器不会产生任何像素；只有相机而没有更新宽高比，窗口改变大小后会出现拉伸。清单\ref{lst:ch06-three-minimal}把这些职责放在一个模块中，并在每一帧读取渲染区域尺寸。\texttt{setAnimationLoop}由Three.js根据当前渲染器选择合适的动画调度方式，在支持 WebXR 的环境中也能复用。

\begin{lstlisting}[label={lst:ch06-three-minimal},language=JavaScript,breaklines=true,caption={Three.js r160+最小可运行渲染骨架}]
import * as THREE from 'three';
...
颜色管理必须区分“纹理已经是 sRGB 编码”和“光照计算使用线性空间”。r152 以后，渲染器输出使用\texttt{outputColorSpace}，常见显示器输出设置为\texttt{THREE.SRGBColorSpace}；旧的\texttt{outputEncoding}与\texttt{sRGBEncoding}不应作为 r160+ 新代码。颜色贴图应标记为 sRGB，法线、粗糙度和金属度贴图保持线性数据。Three.js 当前默认启用\texttt{THREE.ColorManagement.enabled}，但项目仍应在初始化处写出假设并在截图回归测试中固定曝光和色调映射。清单\ref{lst:ch06-color-space}把这些假设写成代码：渲染器输出色彩空间、颜色贴图的 sRGB 标注与法线贴图的线性标注各占一行，便于核对。

\begin{lstlisting}[label={lst:ch06-color-space},language=JavaScript,caption={Three.js色彩空间与纹理颜色标注}]
import * as THREE from 'three';
...
Three.js 自 r156 起在 npm 包的 \texttt{exports} 字段中内置了\texttt{three/addons/*}到\texttt{examples/jsm/*}的映射，因此 Vite 等现代构建工具无需任何配置即可解析\texttt{three/addons/}导入；仍需手工声明别名的只剩“无构建工具、直接用浏览器 importmap”这一种运行方式。两条导入路径指向同一份文件，工程内应统一选用一种写法。清单\ref{lst:ch06-addon-import}列出两种环境的对应关系。
...
官方addon路径为\texttt{three/addons/loaders/GLTFLoader.js}\cite{threeGLTFLoader}。glTF/GLB适合Web交付；加载后仍需检查单位、坐标轴、材质色彩空间和包围盒。清单\ref{lst:ch06-gltf-load}在加载回调里做了这几项检查，并把业务标识写进\texttt{userData}，第7章的拾取就是靠它找回测点的。

\begin{lstlisting}[label={lst:ch06-gltf-load},language=JavaScript,breaklines=true,caption={Three.js 加载 GLTF 工程模型}]
import * as THREE from 'three';
...
\paragraph{业务问题}\texttt{GET /api/assets}返回 28 个对象，每个有经纬度与高程（配套数据集 stations.csv）；场景里要出现 28 个可点击的小球，点到哪个球就要知道它的\texttt{assetId}。本小节只做“挂上去、找得到”，坐标转换的精确做法在 6.2 节，拾取交互在第7章 7.4 节。

\paragraph{做法}三条规则。第一，业务标识写进\texttt{userData.assetId}，永远不用网格名字或数组下标当标识——名字会重复，下标会因为增删而漂移。第二，同类对象放进一个\texttt{Group}，删一次清一组，第4章讲的资源释放在三维里表现为“切换工程时把上一组全部 dispose”。第三，从对象到网格的查找用一张\texttt{Map}，不要每次遍历场景。清单\ref{lst:ch06-bind-assets}把这三条写成一个函数；输入就是契约的\texttt{AssetDto}加上台账里的坐标，输出是可被拾取的组。局部坐标此处简单地把经纬度差乘以米/度换算、高程直接作 y，这种近似适合观察对象绑定；工程定位所需的投影、基准与精度校核见 6.2 节。
...
\paragraph{可观察结果与检查表}调用后坝体附近出现 28 个蓝色小球；控制台执行\texttt{bound.find('DAM-A-PZ-07').position}应得到一个 y 约等于台账中该测点高程的向量。表\ref{tab:ch06-bind-check}是交付前必须逐项核对的清单——对象映射错一个，第7章曲线就会挂到错误的球上，而页面不会报任何错。
...
图\ref{fig:ch06-bound-scene}按编码定位并聚焦 PZ-07；场景实际绑定28个测点。10 m 网格、垂直投影和坐标文字为采集时添加的识读标记，测点坐标与几何保持原样。对象总数与当前可见数量应分别核对。
...
\paragraph{本节层次}核心：6.2.2、6.2.3；指导实践：6.2.1；拓展：6.2.4。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读6.1节核心内容，能辨认场景坐标、相机和对象；复习坐标与高程概念。


\subsection{OGC地图与要素服务}

WMS根据请求范围、尺寸、坐标参考系和样式动态渲染地图图像；WMTS按预定义瓦片矩阵提供地图瓦片，便于缓存和大规模浏览；WFS提供可查询的矢量要素；WCS覆盖栅格数据本体，可按范围和波段取得原始像元值，适合DEM取值与淹没分析\cite{ogcWMS,ogcWMTS,ogcWCS}。3D Tiles是OGC社区标准，按层次细节（LOD）流式加载大规模三维对象，适合把BIM、倾斜摄影和点云交付到浏览器。表\ref{tab:ogc-services}把这些服务按“返回什么”并列：WMS返回按请求参数渲染出来的图像，WMTS返回预先切好的瓦片，WFS返回可查询的矢量要素本身，WCS返回可分析的栅格覆盖，3D Tiles返回带层次结构的三维内容。选型时先确定业务需要的是图像、瓦片、要素、原始像元还是三维流式内容，再决定服务路线。
...
经典OGC服务常用KVP（Key-Value Pair）请求表达操作。下面三条请求分别取得图像、要素和覆盖数据；实际项目还需加入认证、版本、异常格式和服务地址。WMS 1.3.0使用\texttt{CRS}参数而不是旧版本的\texttt{SRS}；当坐标系为EPSG:4490时，规范轴序是纬度在前、经度在后，\texttt{BBOX}应按该顺序填写。客户端若沿用“经度,纬度”的习惯，会得到空白图或位置偏移。清单\ref{lst:ch06-wms-getmap}用于获取底图，其中\texttt{CRS}与\texttt{BBOX}两个参数必须配套。
...
清单\ref{lst:ch06-wfs-getfeature}取的则是矢量要素本身而不是渲染好的图片，返回 GeoJSON 后可以直接参与前端的属性查询与空间判断。
...
注意轴序：EPSG:4490 的官方定义是纬度在前，但 GeoServer 对简写\texttt{EPSG:4490}形式的\texttt{SRSNAME}默认按经度在前处理，本例即按此约定书写；若改用\texttt{urn:ogc:def:crs:EPSG::4490}形式，BBOX 必须交换为纬度在前，这是 WFS 联调中最常见的“查不到要素”原因。第三类是栅格：清单\ref{lst:ch06-wcs-getcoverage}用 WCS 取回带高程数值的覆盖数据，它和 WMS 的区别在于返回的是可参与计算的数值，不是一张看图。
...
复核还应覆盖错误坐标系、越界范围、超大图片、未知图层和过期令牌等异常输入，检查服务是否返回稳定错误码并且不泄露内部路径。异常请求的traceId与发布版本写入审计记录，便于把一次空白地图还原为具体的参数和配置。
对异常响应还要检查Content-Type、错误码和重试建议，前端按契约展示可操作的提示。
服务端同时限制错误详情长度，避免把数据库或文件系统信息暴露给外部用户。
客户端日志只保留必要上下文和traceId。
故障恢复后再开放自动刷新。
恢复记录应关联演练编号和验证人员。
并标注验证结果。
记录已归档并可追溯。
...
教学示例使用自建GeoServer，不依赖商业底图密钥。清单\ref{lst:ch06-cesium-wmts}用自建服务的 WMTS 图层替换掉 Cesium 默认的在线底图，这样课堂环境断网或没有密钥时场景仍然能出图：
...
清单\ref{lst:ch06-geoserver-gridset}是教学配置片段，真实项目需从测区范围和服务规范计算分辨率、矩阵宽高与最大层级。若图层声明的CRS与gridset不一致，缓存会在切片阶段产生错位；若最大层级没有和Cesium的\texttt{maximumLevel}同步，客户端会持续请求不存在的瓦片并制造404噪声。发布脚本应把这两项作为启动前检查。
...
我国大陆范围的6度高斯—克吕格分带为13至23带。经度$\lambda$所在带号可按
\[
  N=\left\lfloor\frac{\lambda}{6}\right\rfloor+1,
  \qquad \lambda_0=6N-3
\]
确定中央经线。大比例尺水利测图还常采用3度带，带号与中央经线为
\[
  n=\left\lfloor\frac{\lambda-1.5}{3}\right\rfloor+1,
  \qquad \lambda_0=3n.
\]
带号是投影分带的计算结果，不是流域名称的固定属性；黄河、长江等跨区域工程应依据每个数据集的经度范围、比例尺和项目坐标规范选择分带。代码不能硬编码某一固定带号，也不能在未标注3度带或6度带口径时引用带号区间。

CGCS2000的EPSG代码还要区分地理坐标、带号前缀和无前缀东坐标。表\ref{tab:ch06-cgcs-epsg}列出本章涉及的范围，工程交付时应在元数据中同时写出EPSG代码、中央经线、带宽、尺度因子和东坐标是否带号前缀。
...
图\ref{fig:ch06-coordinate-chain}展示经纬度到场景坐标的转换。平面投影输出 $E,N$；高程 $H$ 需另行统一垂直基准，再参与局部平移、旋转和轴向换算。带宽、中央经线与场景原点均须记录，缺少参数可能造成系统性错位。
...
清单\ref{lst:ch06-zone-calc}用经度计算两种带宽的带号和中央经线，结果应和坐标数据的EPSG元数据交叉检查。浮点经度靠近分带边界时，要保留原始经度精度并由项目负责人确认是否采用邻带或统一工程坐标。
...
下面示例把CGCS2000经纬度转换到中央经线111度的6度带（19带），采用无前缀东坐标。\texttt{tmerc}表示横轴墨卡托/高斯—克吕格计算；GRS80与CGCS2000椭球参数一致，\texttt{+k\_0}是PROJ 6+推荐写法，\texttt{+no\_defs +type=crs}用于明确坐标定义。带号写入元数据，交付系统若要求带号前缀，再按项目规范转换。清单\ref{lst:ch06-proj4-convert}把这段定义写成 proj4 字符串并做一次往返转换，往返误差应在毫米量级；误差过大通常说明椭球或中央经线填错了。
...
GNSS通常给出椭球高$h$，水利工程常使用1985国家高程基准下的正常高$H_\gamma$。二者通过高程异常$\zeta$联系：
\[
  H_\gamma=h-\zeta.
\]
正常高对应我国采用的正常重力场和高程异常；若业务数据采用正高，则应使用大地水准面差距$N$并计算
\[
  H_g=h-N.
\]
两种高程不能混写为同一个“海拔”字段。淹没线、闸顶和监测点高程必须记录高程类型、基准、单位、转换模型和版本，转换模型由测绘专业人员按区域成果确认。

高程异常具有明显的区域性。按重力似大地水准面模型CNGG2011的统计，我国大陆范围内高程异常约在$-68\,\mathrm{m}$至$+28\,\mathrm{m}$之间\cite{li2012cngg}：负极值出现在受印度洋大地水准面低谷影响的西部地区，正极值出现在东南部；同一测区内部的变化通常在米级到十米级。这里的数值用于帮助学生建立数量级概念，工程计算应采用测区批准的似大地水准面或高程异常模型，例如CNGG2011或省级加密模型，并在元数据中记录模型名称、版本、格网分辨率、适用范围和发布日期。模型文件随成果归档，程序不得用固定常数替代区域模型。

清单\ref{lst:ch06-height-convert}把高程转换写成显式函数，输入值和输出值都保留单位与基准标签；当缺少异常模型时，程序应拒绝静默换算。
...
\paragraph{轴序、单位与序列化}
EPSG定义的坐标轴顺序与许多Web API的习惯并不总是一致。GeoJSON通常以经度、纬度序列化，WMS 1.3.0在某些EPSG坐标系下按规范使用纬度、经度；工程接口应在契约中写明轴序，并使用带名称的对象字段或显式数组顺序。长度统一用米、角度统一用度，角度的度分秒输入必须先转换为十进制度再进行带号计算。数据库字段类型与精度也要与坐标用途匹配：经纬度保留足够小数位，工程平面坐标保留毫米或项目规定的精度，高程字段同时保存数值和基准枚举。清单\ref{lst:ch06-coordinate-validate}把轴序、单位和基准做成必填的元数据字段并在入口处校验，缺项直接拒绝，不让“默认是经度在前”这类假设留在代码里。
...
当平面坐标进入Three.js时，通常要先减去局部原点$(E_0,N_0,H_0)$，再按米制缩放和场景轴约定旋转。局部变换的逆运算必须保留：点击场景中的测站后，系统先把局部坐标加回原点，再转换到EPSG:4490或工程平面坐标，最后调用后端查询。若只保存渲染后的浮点坐标，用户看到的构件无法与测量成果或监测数据库建立可复核关系。清单\ref{lst:ch06-local-origin}把正反两个方向写在一起，正是为了让这条逆运算不被遗漏。

\begin{lstlisting}[label={lst:ch06-local-origin},language=JavaScript,caption={工程坐标与三维局部原点互转}]
...
服务接口的坐标参数应使用结构化对象，例如{\texttt{crs: "EPSG:4490", axisOrder: "lon-lat", coordinates: [113.7,34.2]}}，而不是把“113.7,34.2”拼成无上下文字符串。投影平面接口应另外返回单位、中央经线和带号；客户端把数据转换为局部原点时，将原点、旋转角和缩放系数写入场景配置。这样，三维拾取、二维地图查询和后端空间分析共享同一份可验证的坐标契约。
...
坐标版本更新还应通知地图服务、三维瓦片和监测接口的维护者，形成变更单与回滚点；发布说明列出受影响的图层、测站和查询时间窗，值班员据此安排抽查。
转换链中的每个参数都应能由脚本读取并打印，避免人工复制时丢失小数位或符号。
验收人员依据同一脚本和样本数据复核结果，复核记录与发布版本一并归档。
复核过程同时检查坐标轴方向、单位和高程基准标签，确保数值与语义保持一致。
若标签缺失，数据进入隔离区等待补全后再发布。
补全后重新执行同一套样本校验并更新审核状态。
审核状态还应记录复核人、复核时间和使用的样本版本。
并保留复核结论。
...
Cesium的\texttt{Cesium3DTileset.fromUrl}返回Promise，应等待对象创建完成后再加入场景\cite{cesium3DTileset}；清单\ref{lst:ch06-3dtiles-load}用\texttt{await}而不是已废弃的\texttt{readyPromise}：
...
\paragraph{本节层次}拓展：6.3.1、6.3.2、6.3.3。本节为拓展选读，不作为核心理论的前置。

\paragraph{进入本节所需知识}先读6.1–6.2节核心内容，能核对坐标、尺度与对象标识，再检查外部模型。
...
图\ref{fig:ch06-aerial-control-network}给出了控制网络的教学化布设：连接点负责把相邻影像锁定到同一块地物，像控点把模型约束到工程坐标，检查点保持独立用于验收。控制点应覆盖坝轴线两端、坝顶与坝脚、高程变化明显的坡面、闸室和场景边界；像控点与检查点均应覆盖测区，检查点独立检核且不参与平差；高程上的分布另需结合地形设计。图中不同符号区分点的用途；平差文件还需逐点记录角色、测量方法、坐标基准、精度等级和是否参与平差。
...
成果精度报告应按平面和高程分别给出统计量。式(\ref{eq:rmse-x})和式(\ref{eq:rmse-y})分别描述东、北分量，工程验收通常还需要平面点位均方根误差$ m_p $，其定义为
\begin{equation}
  m_p=\sqrt{m_x^2+m_y^2}.
  \label{eq:rmse-p}
\end{equation}
按式\ref{eq:rmse-p}可将两个平面分量合成为点位指标。因此，限差判断应采用“平面点位均方根误差$ m_p $与高程均方根误差$ m_h $”这一对指标；$m_x$与$m_y$保留为诊断量，用来定位某一方向的系统偏差，不能把两个分量分别与同一条平面限差简单比较。报告还应列出最大绝对误差、点数、点的空间分布、限差来源、坐标/高程基准和软件版本。典型GSD目标与验收口径的关系见表\ref{tab:ch06-aerial-accuracy}，表中的“按规范查表”提醒读者：比例尺、地形类别和成果用途会改变允许误差，示例目标不能替代项目适用条款。
...
清单\ref{lst:ch06-accuracy-check}把分量误差、合成平面误差和粗差标记同时输出，学生可以用同一份检查点 CSV 复核软件报告。代码中的阈值只负责生成复核队列，最终限差仍由项目适用规范和验收文件决定。
...
在成果交付单中，平面限差、高程限差、中误差计算口径和粗差处置规则应作为一个整体签字确认。若项目采用地方细化规范或合同中的更严要求，应在表\ref{tab:ch06-aerial-accuracy}对应行补充版本、条款和适用范围；若规范版本发生变化，重新计算检查点并保留旧版报告，确保三维模型、BIM构件和监测平台引用的是同一成果版本。
...
每次复核还要保存输入清单、软件环境和脚本版本，使同一批检查点能够在另一台机器上得到相同结果。对于跨期更新，报告注明哪些差异来自新采集、哪些差异来自处理参数，避免把版本变化误读为工程变形。
复核结果与发布版本同步归档。

图\ref{fig:oblique-preprocess}展开预处理与质量控制环节。质量判定通过后保存合格影像与报告；缺片、模糊或重叠不足需要补拍、重采，色调或接边问题则返回校正与匀色。返工结果重新经过检查。
...
图\ref{fig:ch06-appearance-components}把同一坝体的外观纹理与构件分区并列。左侧帮助辨认表面和周边环境，右侧用颜色区分闸门、支承结构和启闭设备。颜色分区只是构件识别的入口；能否查询某扇闸门的编码、开度和检修记录，还取决于模型对象与业务数据的绑定。
...
表\ref{tab:ifc-waterworks-mapping}把常见水工对象与 IFC 4.3 的表达边界并列起来。表中枚举值采用 IFC 4.3.2 的定义；标为“无原生实体”的对象，需通过项目分类、属性集和映射规则保留工程语义，并随模型一并交付。
...
水工对象映射到 IFC 时，应依据对象用途与实体定义选择表达方式：闸门应按项目分类和属性集表达，不能用\texttt{IfcDoor}表示；廊道应按空间与连通关系表达，不能用\texttt{IfcFurniture}表示；坝体应按工程对象和项目映射规则表达，不能简单当作\texttt{IfcStructuralMember}。其中\texttt{IfcStructuralMember}属于结构分析模型实体，用于分析构件语义，不是水工物理构件的通用替身。对于船闸闸首，只有在对象确实属于船闸空间部分时，才可依据表\ref{tab:ifc-waterworks-mapping}使用\texttt{GATEHEAD}；该值不能迁移到水库泄洪闸门。
...
\paragraph{本节层次}拓展：6.4.1、6.4.2、6.4.3、6.4.4、6.4.5、6.4.6、6.4.7、6.4.8、6.4.9、6.4.10、6.4.11、6.4.12、6.4.13、6.4.14、6.4.15、6.4.16、6.4.17、6.4.18。本节为拓展选读，不作为核心理论的前置。

\paragraph{进入本节所需知识}先读1.2节、3.2节和6.1节核心内容；本节供模型与平台集成选读。
...
\]
其中$PE$为物理实体、$VE$为虚拟实体、$Ss$为服务、$DD$为孪生数据、$CN$为连接。孪生数据 $DD$ 包括观测、状态和模型运行所需的数据；各维度在水利平台中的对应关系如下图所示。图\ref{fig:dt-five-dimensions}把五个维度画在一起，用连接维 $CN$ 表示其余四维之间的数据交换与反馈关系。连接维承载观测更新与执行反馈；双向连接中断时，虚拟状态可能落后于现场，指令结果也无法得到确认。
...
三维交互的详细视觉编码、LOD降级、键盘可达和图表联动由第7章负责，本节只保留跨层证据链的接口约束；具体展示实现可参见\ref{sec:monitoring-interaction}节。第6章的职责是定义对象、状态、模型证据和处置记录如何连接，第7章的职责是把这些证据转成可读、可操作的二维与三维界面。
...
我国数字孪生水利建设强调流域防洪、水资源管理和工程运行等实际任务\cite{twinWater2023,twinProject2024}。平台规划应依据这些任务所需的观测、模型计算与执行反馈，分阶段配置数据、模型和服务，并同步考虑更新和运维成本。
