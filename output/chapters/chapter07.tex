...
\begin{tipbox}
\paragraph{工程版本线：v3（三维场景接入） $\rightarrow$ v4（观测展示与联动）}起点是 v3 的三维底座。本章结束时你应交付 \textbf{v4}：曲线与三维联动的观测展示——缺测断线、质量码视觉编码、图表点击定位三维测点；仓库 \texttt{frontend/src/utils/readings.js} 的纯函数（缺测保位、时间窗校验）是本章代码的最小起点，测试见 \texttt{frontend/tests/readings.test.js}。
\end{tipbox}

\section{面向可视化的数据准备}\label{sec:data-types-display}

\paragraph{本节层次}核心：7.1.1、7.1.2、7.1.6；指导实践：7.1.3、7.1.4；拓展：7.1.5。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读4.5节核心内容及表\ref{tab:api-contract}，能辨认时间、数值、单位和质量码。

\subsection{从观测记录到视觉变量}

可视化输入不是一串没有语义的数值。每条记录至少要包含工程编码、测点编码、观测时间、监测项、数值、单位、质量码和来源。若数据经过插值、人工修订或模型估计，还应保留处理标记，避免界面把推算值当作原始观测。表\ref{tab:ch7-data-types}按监测项把案例水库的四类观测与网关状态列开，并给出各自适合的展示方式。同样是随时间变化的量，渗压和水位适合连续过程线，雨量适合分时段柱状，设备状态则只能用离散符号——展示方式应与观测量的连续性、累计方式和取值类型相适应。
...
质量码应先于颜色映射。质量码本身只有 valid、suspect、missing 三个取值：有效数据可进入曲线和状态计算，可疑数据保留但降低视觉权重并提示复核，缺测不连接为连续曲线。人工修订另设处理标记，界面显示修订标识；设备故障记录在设备状态中，故障期间的数据不得参与风险判定。颜色只表达业务状态，不能掩盖数据质量。图\ref{fig:data-visualization-process}把这条顺序画成处理链：接入、质检、时间对齐、降采样、视觉映射、渲染，质量判定位于颜色映射之前。未经质检便映射颜色，可能把可疑值显示为正常状态。
...
清单\ref{lst:ch07-lttb}把分桶、下一桶均值和最大面积选择写成可运行的数组函数。图\ref{fig:lttb-principle}用一个桶画出$P_{\mathrm{prev}}$、两个候选点和$\overline{P}_{C}$，半透明三角形表示被选候选点对应的面积；桶边界表示算法的遍历范围，桶内相邻点的实际时间间隔可不同。
...
案例水库的三类主要测项采样周期不同：渗压每1\,h一条，水位每5\,min一条，雨量每5\,min一条。对齐时先选定展示时间格网，再把每个测项映射到格网而不是简单地按数组下标拼接。5\,min格网可以保留水位和雨量的短时响应，渗压在其余时间格中保持空值或沿用“最近观测时间”标记；1\,h格网适合趋势比较，此时雨量应按小时累计，水位应取末值或平均值并注明聚合规则，渗压通常取小时末值。表\ref{tab:ch7-time-alignment}把采样周期、默认聚合和禁用操作列出，读者可以据此检查接口契约是否把瞬时量与累计量混在一起。
...
清单\ref{lst:ch07-resample-align}保存为\texttt{frontend/src/lesson71/resample.js}，只处理同一对象、单位和观测版本中已有输入的桶，不自动生成缺桶。先按实际时刻排序，再取最近可用值；缺测、非有限数和拒收记录均排除，同一时刻重复记录直接报错。雨量输入以时段起点标时，\texttt{sampleMs}给出原始周期；缺少任一时段时，部分累计标为suspect。渗压若在5\,min格网中保持显示，图例仍须标注最近观测时间与年龄。

事件时间\texttt{occurredAt}表示传感器实际观测时间，处理时间\texttt{ingestTime}表示平台收到消息的时间。网络抖动、网关缓存和重试会使二者相差数分钟甚至更久，排序和绘图必须使用事件时间；处理延迟监控则使用处理时间。水印表示事件时间的处理进度。本例以已见最大事件时间减去乱序容忍度推进水印；新记录的事件时间早于此前水印时进入修订队列，其余记录参与当前窗口，不直接覆盖已经确认的预警。修订后的图形要显示“迟到修订”标记并保留前后版本，避免值班员误以为系统曾经实时看到过后来补传的数据。
...
本书在前端示例中使用valid、suspect、missing三个值，是为了让学生看清“是否参与计算”和“如何显示”的关系，属于教学简化。工程落地时应将这三个值映射到水利行业标准规定的质量标识、检验方法和审核状态，并在数据字典中记录映射版本；可参照现行行业标准SL/T 247-2020《水文资料整编规范》中的整编、审核与定级要求执行\cite{slt2472020}，不能把教学枚举直接当作行业标准原码。质量码改变时要产生审计事件，记录操作者、规则版本、原始值和修订值，原始观测永远只读。
...
图\ref{fig:ch07-missing-gap}对照同一组记录的两种画法。在$t_2$、$t_3$保留空值时，曲线中间断开；删除这两条空值记录后，相邻观测点被直接连接。中间的线段没有观测支持，不能据此判断缺测时段的变化。可疑点保留原值，并用菱形提示复核。
...
\begin{tikzpicture}[x=1cm,y=1cm,>=Stealth,font=\scriptsize,line cap=round]
  ...
  \foreach \shift/\heading in {0/{保留空值：缺测处断线},7.7/{删除空值：跨越缺测连线}} {
    \begin{scope}[xshift=\shift cm]
      ...
      \fill[black!8] (2.85,2.3) rectangle (3.85,4.7);
      \node[text=TextGray] at (3.35,4.45) {缺测};
...
清单\ref{lst:ch07-missing-disconnect}按图\ref{fig:ch07-missing-gap}保留缺测位置，使用表达式\texttt{quality === 'missing' ? [t, null] : [t, v]}生成序列。阴影区间由固定时间格网确定，起止时间与横轴对齐。运行后可检查两项结果：缺测位置没有数据点，相邻有效点没有跨空值连接。设备离线造成长缺测时，还应显示网关状态与最后一次有效时间，帮助值班员判断是通信故障还是传感器静默。
...
清单\ref{lst:ch07-quality-check}保存为\texttt{frontend/src/lesson71/quality.js}，保留上游suspect和missing，未知质量码、非有限数、字符串数值、单位或时序错误均留下问题码并拒收。\texttt{rules}由测点台账提供单位及可选量程，示例不另设工程阈值。只有valid且未拒收的记录参与评分；质量码在前端只读，正式订正仍由后端留痕。
...
图\ref{fig:ch07-gap-results}使用配套数据中2026年7月1日的库水位记录。03:05的质量码为\texttt{missing}，值映射为\texttt{null}；该时刻仍占据横轴位置，曲线在两侧断开。
...
清单\ref{lst:ch07-standardize-readings}保存为\texttt{frontend/src/lesson71/standardize.js}，未映射设备、未知单位和非法时间均报错；空值与空字符串归为missing，数字字符串交由来源适配器明确解析。转换保留来源、版本和单位因子，上游可疑值仍为suspect。

三模块的调用见清单\ref{lst:ch07-data-examples}，保存为同目录的\texttt{examples.js}后，在\texttt{frontend/}执行\texttt{node src/lesson71/examples.js}。185091\,Pa来自PZ-07已有教学观测185.091\,kPa的单位换算，仅演示数据语义。输出应保留suspect且禁止评分；未知设备、重复时间与无限值分别触发失败分支。三模块均使用配套\texttt{shared/time.mjs}解析带时区时间，回归验证见\texttt{tests/lesson71.test.js}。
...
案例水库页面使用统一的展示记录：表\ref{tab:ch7-view-contract}列出这条记录的最小字段。其中质量码与处理标记不是可选项：前端拿不到它们，就无法区分“这一段本来就没有观测”和“这一段的观测不可信”，只能把两种情况画成同一条断线。
...
\paragraph{本节层次}核心：7.2.1、7.2.4、7.2.5；指导实践：7.2.2；拓展：7.2.3、7.2.6、7.2.7。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读7.1节核心内容与4.5.4节竞态处理；联动实践复用6.1节场景。
...
图\ref{fig:ch7-rain-level-combo}中，雨量柱从零基线向上绘制，水位线使用独立轴但共享时间索引；若为了突出“倒挂”效果将柱形向下绘制，也要在图例中说明向下方向代表降雨量而不是负值。水位预警由第8章的规则结合质量码与模型版本确定。

ECharts 配置项与 API 随版本演进，本章写法以官方手册为准\cite{echartsDocs}。本章示例统一沿用第4章的 Vite 工程：ECharts 与坐标库通过 \texttt{npm install echarts proj4} 安装、以 \texttt{import * as echarts from 'echarts'} 与 \texttt{import proj4 from 'proj4'} 引入；Three.js 沿用第6章工程的安装与导入方式。无构建环境的课堂演示可改用官方 CDN 的全局构建，此时 \texttt{echarts}、\texttt{THREE}、\texttt{proj4} 为全局变量，使用前应确认这三个变量已由对应脚本加载。清单\ref{lst:ch07-rain-level-dualaxis}给出可运行的雨量倒挂柱与水位过程线组合，显式声明两个y轴、单位、tooltip格式和数据缩放；真实项目还要从服务端响应读取质量码并对缺测点设置空值，不把缺测当作零雨量或零水位。

\begin{lstlisting}[label={lst:ch07-rain-level-dualaxis},language=JavaScript,breaklines=true,caption={雨量倒挂柱与水位过程线ECharts配置}]
...
本书以Apache ECharts为图表主线。ECharts最初由百度团队开源，2018年捐赠给Apache软件基金会，2021年从孵化器毕业为顶级项目，现由ASF社区维护并持续发布版本\cite{apacheECharts}。选用版本时应查阅Apache发布记录和对应的API文档。图\ref{fig:chart-technology-selection}划出三者的职责边界：图表库负责二维统计视图，三维引擎负责场景与空间定位，中间由一个联动控制器负责转发选中对象与时间范围。把联动逻辑写进任意一侧，都会让两个库互相依赖，日后替换其中之一时牵连面很大。Three.js负责三维场景，图表与场景之间不直接互相调用，选中对象和时间范围一律由联动控制器转发。
...
视觉编码的对比度与非颜色冗余要求依据 WCAG 2.2\cite{wcag22}。页面使用“正常、关注、告警、离线”四种展示状态，它们与全书统一口径的对应关系是：正常对应无预警（NONE），关注对应蓝色预警，告警覆盖黄、橙、红三级（详情中展示具体等级），离线不是预警等级，它来自网关或设备状态维度，与质量码、预警级别并列存在。状态颜色需兼顾可访问性：四种状态不能只依赖红绿差异，还应配合形状、图标、线型和文字。阈值带要注明适用工况和版本，不能让一条没有来源的红线替代业务解释。

\subsection{可运行的实时曲线}

ECharts的\texttt{setOption}不是“把新对象覆盖到旧对象”这么简单。默认\texttt{notMerge:false}时，框架会把新配置与旧配置合并；\texttt{series}优先按稳定的\texttt{id}匹配，同一序列只更新被提供的字段。\texttt{lazyUpdate:true}允许把短时间内的多次更新合并到下一帧，适合5\,min采样或批量补传。若业务要删除一条已经不存在的序列，必须显式使用\texttt{replaceMerge:['series']}，否则旧序列可能继续留在图例和tooltip中。本书的水位折线\texttt{line}通过\texttt{setOption}更新。\texttt{appendData}的增量渲染仅适用于支持它的系列，例如基础 ECharts 的\texttt{scatter}和\texttt{lines}，且不支持\texttt{dataset}；其中\texttt{lines}是线段系列，与水位折线\texttt{line}用途不同\cite{apacheECharts}。

表\ref{tab:ch07-echarts-update}对照三种更新方式。选择方式时先问“是否改变序列结构”：只改数据点可局部合并，需要删除序列就替换集合，仅当系列支持增量渲染时才使用追加接口。无论哪一种方式，应用状态都要保存原始缓冲，不能把图表实例的合并结果当作业务状态。
...
\begin{lstlisting}[label={lst:ch07-chart},language=JavaScript,breaklines=true,caption={基于应用状态的实时水位曲线更新}]
...
\paragraph{尾部更新与结构变更}
水位折线收到新观测时，先把记录追加到应用缓冲，再用\texttt{setOption}更新\texttt{line}系列；缓冲负责保持300点窗口。迟到消息应先按事件时间排序、去重，再重建序列。若图表从一条水位曲线切换为水位与雨量两条序列，应在一次\texttt{replaceMerge}中提供完整集合和对应坐标轴。清单\ref{lst:ch07-append-data}给出可独立使用的更新模块。调用方在导入后传入已初始化的图表，收到一条按时间递增的观测就调用\texttt{appendTail}；需要切换序列时调用\texttt{replaceWithTwoSeries}。验证时追加一个\texttt{missing}点，应保留空值并断线；再追加超过300点，缓冲应只保留末尾300点。将折线更新误换为追加接口时，先核对系列类型是否支持增量渲染。
...
图表和三维场景通过稳定的对象标识和时间窗口联动。ECharts点击事件的\texttt{params.data}保存当前点的原始数组或对象，\texttt{params.seriesId}说明来自哪条序列，\texttt{params.dataIndex}用于定位应用状态中的索引；控制器据此取出\texttt{assetId}，再调用三维场景的高亮接口。反向点击三维对象时，从\texttt{object.userData.assetId}查找图表序列和数据索引，使用\texttt{dispatchAction}发送\texttt{highlight}与\texttt{showTip}。多张图表可通过\texttt{echarts.connect([a,b])}或相同的\texttt{group}共享缩放和提示，但对象定位仍由业务控制器完成。
...
\paragraph{S5 阶段包。}清单\ref{lst:ch07-link-controller}中的\texttt{scene}由\texttt{src/lesson74/scene-bus.js}提供事件总线和对象高亮；\texttt{lesson74.html}与\texttt{main.js}完成装配，使用教学接口或 S3 完整后端以及 S4 场景。点球后应显示该对象曲线，点曲线上的数据点应高亮对应测点。\texttt{series-controller.js}负责异步切换，复用4.5.4节的取消请求与序号校验：一开始切换就解绑并清空旧曲线，当前请求成功后再建立联动。

\paragraph{故障验证与自测}先让渗压测点的请求延迟，再立即选择库水位测点，最终曲线、单位和状态文字应全部属于后选对象；接着选择无观测的位移测点，应显示“暂无观测”且旧曲线无法响应点击。断网时应显示失败原因，恢复后重新选择可重试。若旧图恢复或旧监听器仍生效，检查序号校验和解绑是否位于所有返回分支之前。\texttt{tests/series-controller.test.js}覆盖这些异常路径及卸载后迟到的响应，\texttt{tests/lesson74.test.js}验证对象映射、下标越界和拾取。自测：取消请求后为什么还需要序号？答案要点：取消不能撤回已经完成的异步计算，只有当前选择的结果才有更新权限。

水位、雨量和渗压往往分成上下几张图。清单\ref{lst:ch07-connect-charts}用\texttt{echarts.connect}让它们共享缩放和提示，读者拖动其中一张的时间轴，另外几张同步跟随；但对象定位仍由业务控制器完成，图表库不认识\texttt{assetId}。
...
交互事件可能很密集。指针移动使用节流，时间范围查询使用防抖，告警确认则不得被节流丢弃。触控长按必须在抬起或取消时清理定时器，清单\ref{lst:ch07-long-press}把\texttt{touchend}与\texttt{touchcancel}都接上——只处理前者，用户手指滑出屏幕时长按菜单仍会弹出：
...
短按时\texttt{pointerup}的处理器会先取消定时器，因此不会误触发长按；\texttt{pointercancel}覆盖系统手势或窗口切换导致的中断；\texttt{pointerdown}入口先清理旧定时器，保证多指按下时前一个定时器不会失控触发。

图\ref{fig:ch07-scene-chart}展示S5阶段页中的对象与曲线。核对场景中的选择、状态栏的对象编码及纵轴单位，可以发现“选中了一个对象，却显示另一个对象曲线”的错配。

\begin{figure}[htbp]
\centering
\includegraphics[width=16cm]{images/runtime/s5-scene-chart.png}
\caption{三维对象与时序曲线的联合展示（S5阶段页运行截图，显示放大）}
...
\begin{lstlisting}[label={lst:ch07-visual-encoding},language=JavaScript,breaklines=true,caption={按任务选择视觉变量}]
...
清单\ref{lst:ch07-warning-palette}中的两套配置互不覆盖：预警等级提供业务颜色和图标，质量状态提供边框、纹理和文字。\texttt{styleReading}的标签把二者组合为可读短语，详情面板再展开规则版本、检查时刻和处置入口。若颜色加载失败，图标和文字仍能表达状态；若质量服务返回missing，界面不得把它回退为NONE。颜色之外还要有替代文本：清单\ref{lst:ch07-aria-accessible}给图表配上\texttt{aria}描述与数据表格入口，读屏用户不必依赖视觉编码也能取到同一份数值。
...
地图和图表的联动需要一个明确的选择状态对象，例如\texttt{\{assetId, timeRange, qualityFilter, scenarioVersion\}}。图表点击测点后，地图只改变选中符号和视野，不重写原始数据；地图框选多个测点后，图表查询使用同一组assetId和时间窗，并在标题中显示样本数。用户切换场景版本时，选择状态应清空或重新校验，避免把上一版本的测点高亮到新模型。选择状态还要写入操作日志，便于复盘“用户当时看了哪些对象、使用哪一时间窗、是否过滤了可疑数据”。
...
\paragraph{本节层次}核心：7.3.3；指导实践：7.3.1；拓展：7.3.2。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读6.1.5节对象绑定和7.1节数据准备，按对象标识核对观测。

\subsection{坐标转换与局部原点}

国内水利工程的主线是CGCS2000地理坐标或其高斯--克吕格投影坐标。CGCS2000地理二维坐标系代码为EPSG:4490\cite{epsg4490}。沿用第6章的分带、中央经线和高程基准，可将工程平面坐标换算为三维场景中的局部坐标。表\ref{tab:ch7-coordinate-units}把这一过程涉及的四种量并列：地理坐标以度为单位，投影坐标与场景世界坐标以米为单位，屏幕坐标以像素为单位。多数定位错误源于跨栏套用——例如把度当作米直接相减，或用像素半径去做世界空间的距离判断。
...
负号来自Three.js常用的相机与地面坐标约定，项目也可采用其他轴向，但数据、模型和交互必须统一。清单\ref{lst:ch07-coordinate-transform}把上式实现为一对互逆函数，并把局部原点作为参数传入而不是写死成常量，换工程时只改调用处。
...
国内测绘成果有时把带号前缀拼接到东坐标左侧。例如3度带39号的成果可能写成\texttt{39500000.00}，其中前两位是带号，参与局部原点相减的东坐标是\texttt{500000.00}。如果把带号坐标直接送入场景，局部坐标会突然增加三千多万米，单精度顶点和相机裁剪都会失效。剥离带号必须依据元数据中的带宽和带号，不要按字符串长度盲切；剥离后的结果要与中央经线、\texttt{+x\_0=500000}和控制点复核。清单\ref{lst:ch07-strip-zone-prefix}按这个原则实现：带号来自元数据参数，函数在数值不落在合理范围时直接抛错，而不是返回一个看起来正常的坐标。
...
\]
因此向北位移的$z$分量为负，向东位移的$x$分量为正。角度必须先由度转换为弧度，箭头长度还应设置可视化比例并在图例中标注真实位移，避免把放大的绘制长度误读成工程量。清单\ref{lst:ch07-displacement-vector}把方位角和位移量转成场景向量，缩放比例作为单独参数传入，绘制长度与真实位移因此始终可以互相反算。
...
案例水库的28个观测点规模较小，可以逐点绘制；流域级场景可能包含成千上万个对象，需要LOD和聚合。近距离显示图标、方向和标签，中距离只显示状态符号，远距离按空间网格或工程对象聚合。LOD阈值应按屏幕占用和交互任务实测，不以固定“米数”套用所有相机。图\ref{fig:lod-cluster}把三档表达画在同一场景中：近处逐点带标签，中距离只保留状态符号，远距离按空间网格聚合并标注数量。切换阈值应当用实际屏幕占用和交互任务实测确定，图中标注的距离只是示意。
...
性能预算可以用屏幕占用而不是固定米数表达。测点圆点直径低于3像素时，增加几何细节没有收益；文字标签低于8像素时，应隐藏文字并保留可访问的列表入口；聚合圆点达到20像素以上且重叠严重时，优先显示数量和最高风险。每一级记录对象数、三角形数、材质切换次数和帧时间，按普通笔记本、值班室大屏和移动设备分别验收。这样，LOD阈值来自任务和设备能力，而不是照抄某个项目的距离常数。清单\ref{lst:ch07-world-radius}做的就是这个换算：给定像素预算和当前相机距离，反解出聚合与拾取应当使用的世界半径。
...
\paragraph{本节层次}核心：7.4.1、7.4.3；指导实践：7.4.4；拓展：7.4.2。核心阅读按所列小节推进，实践成果按章末要求验收。

\paragraph{进入本节所需知识}先读6.1节场景基础与4.4节事件；使用已绑定测点的 S4 场景。

\subsection{射线拾取与安全的向量运算}

屏幕点击首先转换为归一化设备坐标，再由相机生成射线，与场景对象求交。求交得到的是网格，不是业务对象：从网格找回测点靠第6章清单\ref{lst:ch06-bind-assets}写入的\texttt{userData.assetId}，再用同一份\texttt{byId}映射反查，两者缺一都会把曲线挂到错误的球上。Three.js的\texttt{Raycaster}已经处理常见网格与点对象；拾取器在构造时保存场景引用，并在方法中通过该引用访问可拾取对象，使依赖关系显式、便于测试和复用。
...
清单\ref{lst:ch07-point-picker}把上面几步收进一个类：构造时保存相机、场景和画布，\texttt{pick}只接收客户端坐标，换画布尺寸或设备像素比时调用方不必改动。拾取测试要覆盖画布边缘、窄屏旋转、设备像素比为2的屏幕、对象被遮挡和相邻对象重叠。测试输入保存客户端坐标、画布矩形、相机参数和预期\texttt{assetId}，而不是只保存一张截图。若模型分块异步加载，点击发生在分块尚未到达时，界面应显示加载状态并允许重试；加载完成后用同一屏幕坐标重新计算射线，不要沿用旧的交点距离。
...
若教学中手工实现M\"oller--Trumbore三角形求交，必须复制向量后再做减法、叉乘和缩放，避免破坏顶点与射线输入。函数返回的是交点的世界坐标，不是参数$t$；若\texttt{direction}没有归一化，$t$表示沿射线方向的缩放量而不等于米制距离，按交点远近排序时应使用\texttt{origin.distanceTo(point)}或先归一化方向向量。清单\ref{lst:ch07-triangle-intersect}是这样一份实现，注意每次运算都先\texttt{clone}再改，输入的顶点与射线不被修改：
...
\texttt{raycaster.params.Points.threshold}使用世界单位，不是像素。触控目标希望具有20像素左右的可点范围时，先按相机距离和视场角换算，清单\ref{lst:ch07-pixels-to-world}给出这段换算——相机拉远后阈值要跟着变大，写死一个常数会让远处的测点点不中：

\begin{lstlisting}[label={lst:ch07-pixels-to-world},language=JavaScript,caption={屏幕像素到世界长度的换算}]
