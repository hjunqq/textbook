...
图形管线的每一级都应有可观测的输入和输出。模型矩阵错误通常表现为整体位置或朝向异常，观察矩阵错误表现为相机方向和“上下颠倒”，投影矩阵错误则表现为远近比例、裁剪或深度顺序异常。把问题定位到管线阶段，比盲目调节模型坐标更可靠。图6.1以同一橙色顶点展示构件、相机视锥与屏幕像素的关系，底部列出坐标变换与裁剪步骤；视口变换得到连续窗口坐标，后续光栅化再生成片元。

<figure markdown>
![图6.1](images/chapter06_fig_6_1.svg)
<figcaption>图 6.1  同一顶点从工程构件到屏幕窗口坐标的变换</figcaption>
</figure>
...
图6.3按编码定位并聚焦 PZ-07；场景实际绑定28个测点。10 m 网格、垂直投影和坐标文字为采集时添加的识读标记，测点坐标与几何保持原样。对象总数与当前可见数量应分别核对。
...
图6.4展示经纬度到场景坐标的转换。平面投影输出 $E,N$；高程 $H$ 需另行统一垂直基准，再参与局部平移、旋转和轴向换算。带宽、中央经线与场景原点均须记录，缺少参数可能造成系统性错位。
...
图6.6给出了控制网络的教学化布设：连接点负责把相邻影像锁定到同一块地物，像控点把模型约束到工程坐标，检查点保持独立用于验收。控制点应覆盖坝轴线两端、坝顶与坝脚、高程变化明显的坡面、闸室和场景边界；像控点与检查点均应覆盖测区，检查点独立检核且不参与平差；高程上的分布另需结合地形设计。图中不同符号区分点的用途；平差文件还需逐点记录角色、测量方法、坐标基准、精度等级和是否参与平差。
...
图6.7展开预处理与质量控制环节。质量判定通过后保存合格影像与报告；缺片、模糊或重叠不足需要补拍、重采，色调或接边问题则返回校正与匀色。返工结果重新经过检查。
...
平台可划分为物理对象层、感知连接层、数据底板层、模型与知识层、服务层和交互应用层。各层分别承担数据校验、模型运行、结果解释与指令审批等职责。图6.10把这六层自下而上排开，双向箭头表示层间接口与数据交换。层间接口规定输入、输出和失败响应；保持接口兼容时，可替换一层的实现而继续使用相邻层。
...
一次完整闭环包括“感知—质检—状态更新—模型计算—风险解释—人工确认—指令下发—执行反馈—效果评估”。并非所有场景都允许自动控制：大坝安全和防洪调度通常需要权限、会商和人工确认。图6.12把这些环节连成回路，并标出人工确认与审批的位置。指令下发后还需核对执行回执，并用后续观测评估效果；缺少任一项时，只能记录已完成的步骤，无法确认执行结果。
...
控制命令比状态事件要求更严格。命令包含发起人、审批记录、目标设备、期望状态、有效期和幂等键；执行端返回接收、拒绝、执行中、成功或失败状态。超时不等于未执行，调用方必须查询最终状态，避免重复启闭设备。图6.13把状态事件与控制命令两条链路并排画出，并标出各自写入审计的位置。状态事件需要消费者去重，防止重复计算或写入；控制命令还须校验审批记录、有效期与幂等键，防止重复投递引起设备二次动作。
