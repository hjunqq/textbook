...
核心：7.1.1、7.1.2、7.1.6；指导实践：7.1.3、7.1.4；拓展：7.1.5。核心路线只要求读完核心小节。
...
核心：7.2.1、7.2.4、7.2.5；指导实践：7.2.2；拓展：7.2.3、7.2.6、7.2.7。核心路线只要求读完核心小节。
...
当消息以严格递增的事件时间到达时，可以使用`appendData`追加尾部点；迟到消息、删除测项和改变时间窗则必须回到应用状态重建序列并调用`setOption`。`appendData`不会替你去重，也不会自动删除旧点，300点窗口仍由状态层或服务端维护。若同一图表从一条曲线切换为“水位+雨量+阈值带”，应在一次`replaceMerge`中提交完整序列集合，避免旧的图例项留在界面。清单7.10把这两条路径写在一起：尾部追加走`appendData`，结构变更走`replaceMerge`，判断依据是序列集合有没有变，而不是数据量多少。
...
清单7.12里的`scene`是应用层封装，配套工程把它实现在`src/lesson74/scene-bus.js`：一个事件总线加`focusAsset`，不`import three`，因此可以在没有 WebGL 的环境里测试。整页装配见`lesson74.html`与`src/lesson74/main.js`，运行依赖是 S3（能返回观测的接口）与 S4（已绑好测点的场景）：点场景里的球，右侧曲线切到该测点；点曲线上的数据点，对应的球变橙。三处边界值得亲手试：图表下标越界时控制器安静返回，不能高亮一个错误的球；曲线里没有该对象时不发`dispatchAction`；换对象时必须先调用上一次返回的解绑函数，否则点击处理器一层层叠加，点一次会跳好几次。`tests/lesson74.test.js`用两个假对象覆盖了这三条。
...
核心：7.3.3；指导实践：7.3.1；拓展：7.3.2。核心路线只要求读完核心小节。
...
核心：7.4.1、7.4.3；指导实践：7.4.4；拓展：7.4.2。核心路线只要求读完核心小节。
