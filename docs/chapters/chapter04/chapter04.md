...
核心：4.4.1、4.4.2、4.4.3、4.4.4、4.4.5、4.4.7、4.4.8；指导实践：4.4.6。核心路线只要求读完核心小节。
...
核心：4.6.1、4.6.2；指导实践：4.6.3、4.6.4。核心路线只要求读完核心小节。
...
指导实践：4.7.1、4.7.2、4.7.3、4.7.4。本节没有核心小节，课堂核心路线可整体跳过。
...
核心：4.8.1；指导实践：4.8.2；拓展：4.8.3。核心路线只要求读完核心小节。

### 4.8.1 阶段验收：S1 与 S2 的检查单

本章的产出分两个阶段验收。S1 是不依赖任何框架和后端的静态监测页面（4.2–4.4 节），S2 是跑在教学接口上的 Vue 页面（4.5–4.7 节）。表4.9列出每个阶段必须能演示的行为和一个故意注入的故障；验收看的是行为与证据，不是代码行数。两个阶段都通过后，第5章写出真实后端，S2 的页面不改代码直接换数据源，这就是 S3 的起点。
...
| S2   | 登录后从教学接口读取对象列表与最新观测；四种页面状态可观察；快速切换对象不出现旧数据；路由可直接打开详情深链接 | `teach=delay:3000` 与 `teach=unauthorized` | “网络”面板中被取消的请求；401 后回到登录页并能回跳；`lesson45.test.js` 通过 |
