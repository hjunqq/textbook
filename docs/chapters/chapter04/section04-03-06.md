...
1. **基础UI组件**：封装基本的UI元素，如按钮、输入框、选择器等
2. **业务组件**：封装特定业务逻辑的组件，如水位图表、流量监测卡片等
3. **页面组件**：组合多个业务组件，实现完整的页面功能
...
1. **按需加载ECharts组件**：减小包体积
2. **数据下采样**：对于长时间序列数据，可以进行下采样
3. **分段加载**：超大数据集采用分段请求和加载策略
...
export default {
  ...
  computed: {
    ...
    keyStations() {
      return this.stations.filter(station => station.isKeyStation)
    },
  ...
  mounted() {
    ...
    this.wsService.on('connect', () => {
      ...
      // 获取所有关键站点ID并订阅
      const stationIds = this.keyStations.map(station => station.id)
      this.wsService.subscribeToStations(stationIds)
    })
