...
HTML（HyperText Markup Language，超文本标记语言）是用来创建网页内容结构的标记语言。它不是编程语言，而是一�?*标记语言**，通过使用一系列**元素（elements�?*来标记不同类型的内容，告诉浏览器如何显示这些内容�?
...
```html
<img src="water-level.jpg" alt="水位监测�? width="300" height="200">
...
| 标签�?| 语义含义 | 应用场景 | 水利平台应用示例 |
|--------|----------|----------|------------------|
| `<header>` | 页面或区域头�?| 网站标题、导航、面包屑 | 监测平台标题、用户信息区�?|
| `<nav>` | 导航链接 | 主导航、分页、目�?| 功能模块导航、报表章节导�?|
| `<main>` | 主要内容 | 页面核心内容区域 | 水文数据展示区、地图显示区 |
| `<section>` | 内容段落 | 逻辑相关的内容分�?| 不同监测指标的数据段�?|
| `<article>` | 独立文章 | 完整的内容单�?| 单个监测报告、新闻公�?|
| `<aside>` | 侧边信息 | 补充说明、相关链�?| 监测点详情、技术说�?|
| `<footer>` | 页面底部 | 版权信息、联系方�?| 数据来源声明、更新时�?|
| `<figure>` | 媒体内容 | 图片、图表、代码块 | 水位曲线图、工程照�?|
| `<figcaption>` | 媒体说明 | 图片标题、图表描�?| 图表标题、数据说�?|
| `<time>` | 时间日期 | 时间标记 | 监测时间、数据更新时�?|
| `<mark>` | 高亮文本 | 强调、搜索结�?| 异常数据标记、警告信�?|
...
<main>
    <h1>水位监测报告</h1>
    <p>本报告包含过�?4小时的水位变化数据，为水利管理决策提供科学依据�?/p>
    <!-- 主要内容区域 -->
</main>
```
...
<article>
    <header>
        <h2>洪水预警公告</h2>
        <p>发布时间�?024-03-15 12:00</p>
        <p>预警等级：橙色预�?/p>
    </header>
    <p>根据气象部门预报，预计未�?小时内，流域上游将有中到大雨，降雨量预计达到50-80毫米。请相关部门密切关注水位变化，做好防洪准备工作�?/p>
    <footer>
        <p>发布单位：水文监测中�?/p>
        <p>联系电话�?00-1234-5678</p>
    </footer>
</article>
```
...
<!-- 页面底部 -->
<footer>
    <p>&copy; 2024 水利监测平台 版权所�?/p>
    <p>数据来源：国家水文信息中�?| 技术支持：水利信息化中�?/p>
    <p>联系电话�?00-1234-5678 | 邮箱：support@water.gov.cn</p>
</footer>

<!-- 文章底部 -->
<article>
    <h2>月度水情分析报告</h2>
    <p>本月全流域降水量较去年同期增�?5%，各主要控制站水位均在正常范围内...</p>
    <footer>
        <p>报告编制：张工程�?| 技术审核：李主�?/p>
        <p>报告日期�?024�?�?5�?| 下次更新�?024�?�?5�?/p>
    </footer>
</article>
```

#### 8. `<figure>` �?`<figcaption>` 标签 - 图表内容
...
<figure>
    ...
    <figcaption>
        �?：黄河花园口�?4小时水位变化趋势�?
        <br>数据来源：水文监测中�?| 更新时间�?024-03-15 14:30
    </figcaption>
</figure>

<figure>
    <img src="dam-inspection.jpg" alt="大坝安全检查现场照�? width="500" height="300">
    <figcaption>
        某水库大坝春季安全检查现�?
        <br>拍摄时间�?024�?�?5�?| 拍摄地点：大坝左岸观测点
    </figcaption>
</figure>
```
...
<!-- 具体时间�?-->
<p>数据更新时间�?
    <time datetime="2024-03-15T14:30:00+08:00">2024�?�?5�?14:30</time>
</p>

<!-- 仅日�?-->
<p>监测日期�?
    <time datetime="2024-03-15">2024�?�?5�?/time>
</p>

<!-- 相对时间 -->
<p>发布�?
    <time datetime="2024-03-15T12:00:00" title="2024�?�?5�?12:00">2小时�?/time>
</p>
```
...
```html
<p>当前水位<mark class="warning">85.23�?/mark>，已接近警戒水位86.00米，请密切关注�?/p>

<p>监测状态：<mark class="alert">需要重点关�?/mark></p>

<p>�?<mark>流量监测</mark>"相关记录中找�?0条匹配结果�?/p>

<p>本次检查发现大�?mark class="important">渗流量异常增�?/mark>，建议立即进行详细检测�?/p>
```
...
| 输入类型 | 功能描述 | 主要属�?| 水利应用场景 |
|----------|----------|----------|-------------|
| `email` | 邮箱地址输入 | `required`, `placeholder` | 用户注册、通知设置 |
| `url` | 网址输入 | `required`, `placeholder` | 外部链接、参考资�?|
| `number` | 数值输�?| `min`, `max`, `step` | 水位数据、流量参�?|
| `range` | 滑动条选择 | `min`, `max`, `step`, `value` | 阈值设置、参数调�?|
| `date` | 日期选择 | `min`, `max`, `value` | 查询时间、监测日�?|
| `time` | 时间选择 | `min`, `max`, `step` | 具体时刻、时间段 |
| `datetime-local` | 本地日期时间 | `min`, `max`, `step` | 完整时间戳输�?|
| `color` | 颜色选择 | `value` | 图表配色、主题设�?|
| `search` | 搜索�?| `placeholder`, `results` | 站点搜索、数据查�?|
...
`time`输入类型专门用于时间信息的输入，通常显示为时间选择器，支持小时和分钟的选择，也可以包含秒的选择。时间格式遵�?4小时制的HH:MM或HH:MM:SS格式，确保了时间表示的标准化和国际化�?
...
该类型的值格式为ISO 8601的本地时间格式（YYYY-MM-DDTHH:MM），注意这里�?本地"意味着不包含时区信息，时间基于用户的本地时区。这种设计简化了时间处理，特别适合于不涉及跨时区操作的本地化应用�?
...
`color`输入类型提供了颜色选择功能，通常显示为一个颜色按钮，点击后会弹出颜色选择器。用户可以通过可视化的颜色面板选择颜色，也可以直接输入十六进制颜色值。这种输入类型返回的值始终是十六进制格式的颜色代码（�?4a90e2）�?
...
`search`输入类型专门为搜索功能设计，在外观和行为上与普通的文本输入框相似，但具有一些搜索相关的特殊特性。在支持该类型的浏览器中，搜索框通常会显示一个清除按钮（×），允许用户快速清空搜索内容。在移动设备上，虚拟键盘会显�?搜索"按钮而不�?回车"按钮�?
...
| 标签�?| 功能 | 常用属�?| 水利应用场景 |
|--------|------|----------|-------------|
| `<video>` | 视频播放 | `src`, `controls`, `autoplay`, `loop`, `muted`, `poster` | 现场监控、工程录像、教学视�?|
| `<audio>` | 音频播放 | `src`, `controls`, `autoplay`, `loop`, `muted` | 语音播报、报警音�?|
| `<source>` | 媒体�?| `src`, `type`, `media` | 多格式兼容、响应式媒体 |
| `<track>` | 字幕轨道 | `src`, `kind`, `srclang`, `label` | 视频字幕、说明文�?|
| `<canvas>` | 画布绘图 | `width`, `height` | 数据图表、地图绘�?|
| `<svg>` | 矢量图形 | `width`, `height`, `viewBox` | 图标、流程图、技术图�?|
...
浏览器在处理`<source>`标签时会按照声明顺序逐个检查每个媒体源，选择第一个它能够支持的格式进行播放。这种机制称�?渐进增强"，它不仅提高了媒体内容的兼容性，还可以根据用户设备的能力和网络条件提供不同质量的媒体文件�?
...
`<canvas>`标签提供了一个可通过脚本（通常是JavaScript）进行动态绘制的图形画布。它是HTML5中最强大的图形处理功能之一，支�?D图形绘制、图像处理、动画制作等复杂的图形操作。画布是基于像素的，这意味着一旦绘制完成，图形就成为了像素数据而不是对象�?
...
<div class="video-controls">
    ...
    <button onclick="seekTo(30)">跳转�?0�?/button>
...
function changeVolume(volume) {
    ...
    console.log('音量设置�?', video.volume);
}

function seekTo(seconds) {
    video.currentTime = seconds;
    console.log('跳转�?', seconds + '�?);
}
...
// 视频事件监听
video.addEventListener('loadstart', () => console.log('开始加载视�?));
...
<div class="alert-controls">
    <button onclick="playAlert('warning')">一般预�?/button>
    <button onclick="playAlert('danger')">危险预警</button>
    <button onclick="playAlert('emergency')">紧急预�?/button>
    <button onclick="stopAllAlerts()">停止所有警�?/button>
</div>
...
HTML5的图形处理能力主要体现在Canvas和SVG两种技术上。Canvas（画布）提供了基于像素的图形绘制能力，通过JavaScript API可以进行复杂�?D图形绘制、图像处理和动画制作；SVG（可缩放矢量图形）则提供了基于矢量的图形描述能力，能够创建可缩放、可交互的矢量图形。这两种技术各有特点，Canvas适合处理复杂的图像效果和高性能的动画，SVG适合创建清晰的图标、图表和可交互的图形元素�?
