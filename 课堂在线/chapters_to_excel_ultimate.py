...
# -*- coding: utf-8 -*-
"""
智慧水利平台架构与开发 - 课堂在线知识点导入转换器（终极修复版）
彻底解决第5章和第6章解析问题
"""
...
def parse_markdown_tables_ultimate(content):
    """终极版解析markdown内容中的所有表格"""
        ...
        if line.startswith('| 一级知识点'):
            ...
            while i < len(lines):
                ...
                if (line.startswith('---') or 
                    line.startswith('>') or 
                    (line.startswith('#') and ('章' in line or '节' in line))):
                    break
...
def process_chapter_file_ultimate(file_path):
    """处理单个章节文件，终极版本"""
    try:
        ...
        # 解析表格
        all_data_rows = parse_markdown_tables_ultimate(content)
        ...
        if len(all_data_rows) > 0:
            ...
            for i, row in enumerate(all_data_rows[:10]):
                ...
                for j, cell in enumerate(row[:7]):
                    if cell.strip():
                        knowledge_parts.append(f"L{j+1}:{cell[:20]}")
                
                attributes = [f"{j+8}:{cell[:15]}" for j, cell in enumerate(row[7:15]) if cell.strip()]
                
                print(f"    行{i+1}: {knowledge_parts} | 属性: {attributes}")
...
def main():
    ...
    for chapter_file in chapter_files:
        ...
        if file_path.exists():
            print(f"\n📖 处理文件: {chapter_file}")
            chapter_rows = process_chapter_file_ultimate(file_path)
    ...
    # 输出Excel文件
    output_file = base_dir / "智慧水利平台架构与开发_课堂在线导入表格_终极版.xlsx"
    ...
    try:
        ...
        for chapter_file in chapter_files:
            ...
            if file_path.exists():
                ...
                chapter_data[chapter_name] = len(chapter_rows)
        
        for chapter, count in chapter_data.items():
            print(f"  📄 {chapter}: {count} 行")
        
        print(f"\n✅ 请检查生成的Excel文件，确保符合课堂在线导入要求！")
