...
# -*- coding: utf-8 -*-
"""
智慧水利平台架构与开发 - 课堂在线知识点导入转换器（完整修复版）
彻底解决第5章和第6章解析问题，生成完整版本
"""
...
def parse_markdown_tables_complete(content):
    """完整版解析markdown内容中的所有表格"""
    ...
    while i < len(lines):
        ...
        if line.startswith('| 一级知识点'):
            ...
            while i < len(lines):
                ...
                # 真正的表格结束标志：
                # 1. 遇到新的章节标题（以##开头且包含数字）
                # 2. 遇到>符号开头的说明文字
                # 3. 到达文件末尾
                if ((line.startswith('##') and re.search(r'\d+\.\d+', line)) or 
                    line.startswith('>') or 
                    i >= len(lines) - 1):
                    break
...
def process_chapter_file_complete(file_path):
    """处理单个章节文件，完整版本"""
    try:
        ...
        # 解析表格
        all_data_rows = parse_markdown_tables_complete(content)
...
def main():
    ...
    for chapter_file in chapter_files:
        ...
        if file_path.exists():
            print(f"\n📖 处理文件: {chapter_file}")
            chapter_rows = process_chapter_file_complete(file_path)
    ...
    # 输出Excel文件
    output_file = base_dir / "智慧水利平台架构与开发_课堂在线导入表格_完整修复版.xlsx"
    ...
    try:
        ...
        for chapter, count in chapter_stats.items():
            if chapter in ['5章', '6章']:
                # 高亮显示修复的章节
                print(f"  🔧 {chapter}: {count} 行 (已修复)")
            else:
                print(f"  📄 {chapter}: {count} 行")
        
        print(f"\n✅ 完整修复版本已生成！")
        print(f"🎯 第5章和第6章的内容已完全解析")
