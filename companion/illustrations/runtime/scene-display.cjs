...
module.exports = async function prepareSceneDisplay(page, { linked = false } = {}) {
  return page.evaluate(async ({ linked }) => {
    ...
    const target = selected.clone().add(new THREE.Vector3(0, -14, 0));
    camera.position.copy(target).add(new THREE.Vector3(70, 42, 92));
    ...
    html.innerHTML = `<div style="position:absolute;left:24px;top:20px;font-size:24px;font-weight:600">${title}</div>
      ...
      <div style="position:absolute;left:24px;bottom:20px;font-size:18px;line-height:1.55">Y 轴向上 · 球体代表测点<br>网格间距 10 m · 水平参考面 y = ${groundY.toFixed(0)} m</div>
    ...
    return { selectedAssetId: mesh.userData.assetId, actualAssets: group.children.length, coordinate: selected.toArray(), groundY, gridSpacing: 10, geometryAndCoordinatesUnchanged: true, click, viewport: { width, height }, annotation: '采集时添加局部参考网格、投影线与编码标签；调整相机；不代表阶段页默认外观' };
  }, { linked });
};
